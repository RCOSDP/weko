...
def test_show_term_and_condition(app, database, client, es_clear):
    ...
    with mock.patch('weko_workflow.views.item_login', return_value=steps):
        ...
        res2 = client.post('/workflow/activity/detail/A-20200108-00100?status=ToDo',
        ...
        activity = _Activity.query.filter_by(activity_id='A-20200108-00100').one_or_none()
        print(activity.activity_id)
        assert activity.activity_confirm_term_of_use == True
        # Login with appover account
        login_user_via_session(client, 2)
        res1 = client.get('/workflow/activity/detail/A-20200108-00100?status=ToDo')
        assert res1.status_code == 200
