...
def insert_user_to_db(database):
    ...
    admin = User(email='info@inveniosoftware.org', password='123456',
                 active=True,
                 roles=[admin_role])
    admin2 = User(email='admin@inveniosoftware.org', password='123456',
                  active=True,
                  roles=[admin_role])
    admin_profile = UserProfile(user_id=1, user=admin, _username='admin',
                                position='Professor')

    non_admin = User(email='non_admin@invenio.org', password='123456',
                     active=True)
    non_admin_profile = UserProfile(user_id=2, user=non_admin,
                                    _username='user1', position='Professor')
    non_admin2 = User(email='non_admin2@invenio.org', password='123456',
                      active=True)
    non_admin_profile2 = UserProfile(user_id=3, user=non_admin2,
                                     _username='user2', position='Professor')
...
def insert_item_type_name(database, name, has_site_license, is_active):
    ...
    item_name = ItemTypeName(name=name, has_site_license=has_site_license,
                             is_active=is_active)
...
def insert_item_type(database, name_id, schema, form, render, tag, version_id,
    ...
    item_type = ItemType(name_id=name_id, harvesting_type=False, schema={},
                         form={}, render={}, tag=tag, version_id=version_id,
...
def insert_access():
    """Insert_access."""
    run_sql(
        "INSERT INTO access_actionsroles(action,exclude,role_id) "
        "values ('superuser-access','f',1)")
    run_sql(
        "INSERT INTO access_actionsroles(action,exclude,role_id) "
        "values ('item-access','f',1)")


def insert_activity_backup():
    """Insert_activity_backup."""
    run_sql(
        "INSERT INTO workflow_workflow(id, flows_id, status, created, updated,"
        "flows_name, itemtype_id, flow_id, index_tree_id) values (24,"
        "'79713a4d-1ef1-4661-b73c-a4d0975b086e','N', now(), now(), 'Life Usage"
        " Application - Student', 1, 1, 1578472847540)")
    run_sql(
        "INSERT INTO workflow_activity(id, activity_id, status, workflow_id,"
        "flow_id,created, updated, activity_start) values (1,"
        "'A-20200113-00004', 'N', 24, 1, now(), now(), now())")
    run_sql(
...
def insert_action_to_db(database):
    ...
    action_end = Action(status='N', id=2, created='2019-11-20 12:06:14',
                        ...
                        action_desc='Indicates that the action has '
                                    'been completed.',
    ...
    action_item_registration = Action(status='N', id=3,
                                      ...
                                      action_version='2018-05-15 00:00:00',
                                      action_lastdate='2018-05-15 00:00:00')
    action_approval = Action(status='N', id=4, created='2019-11-20 12:06:14',
                             updated='2019-11-20 12:06:14',
                             action_name='Approval',
                             action_desc='Approval action for approval '
                                         'requested items.',
    ...
    action_grant = Action(status='N', id=7, created='2019-11-20 12:06:14',
                          ...
                          action_desc='Select DOI issuing organization '
                                      'and CNRI.',
...
def insert_flow_action1():
    """Insert_flow_action1."""
    run_sql("INSERT INTO public.workflow_flow_action(status, created, updated,"
            "id, flow_id, action_id, action_version, action_order,"
            "action_condition, action_status, action_date) VALUES('N', "
            "'2020-01-08 08:19:33.493', '2020-01-08 08:19:33.493', 1, "
            "'2b40b793-0414-4e1b-8704-1314c093394b', 1, '1.0.0', 1, NULL, "
            "'A', '2020-01-08 08:19:33.493')")
    run_sql("INSERT INTO public.workflow_flow_action(status, created, updated,"
            "id, flow_id, action_id, action_version, action_order, "
            "action_condition, action_status, action_date) VALUES('N', "
            "'2020-01-08 08:20:52.495', '2020-01-08 08:20:52.496', 3, "
            "'2b40b793-0414-4e1b-8704-1314c093394b', 8, '1.0.0', 2, NULL, "
            "'A', '2020-01-08 08:20:52.496')")
    run_sql("INSERT INTO public.workflow_flow_action(status, created, updated,"
            "id, flow_id, action_id, action_version, action_order, "
            "action_condition, action_status, action_date) VALUES('N', "
            "'2020-01-08 08:20:52.502', '2020-01-08 08:20:52.502', 4, "
            "'2b40b793-0414-4e1b-8704-1314c093394b', 11, '1.0.0', 3, NULL, "
            "'A', '2020-01-08 08:20:52.502')")
    run_sql("INSERT INTO public.workflow_flow_action(status, created, updated,"
            "id, flow_id, action_id, action_version, action_order, "
            "action_condition, action_status, action_date) VALUES('N', "
            "'2020-01-08 08:19:33.495', '2020-01-08 08:20:52.505', 2, "
            "'2b40b793-0414-4e1b-8704-1314c093394b', 2, '1.0.0', 4, NULL, "
            "'A', '2020-01-08 08:19:33.495')")


def insert_flow_define():
    """Insert_flow_define."""
    run_sql("INSERT INTO public.workflow_flow_define(status, created, updated,"
            " id, flow_id, flow_name, flow_user, flow_status) VALUES('N', "
            "'2020-01-08 08:33:42.014', '2020-01-14 07:04:50.432', 5, "
            "'18f974eb-bb67-446e-9ce1-db57110a74ca', 'JGSSデータ登録', 1, 'A')")
...
def insert_flow_role():
    ...
    run_sql("INSERT INTO public.workflow_flow_define(status, created, updated,"
            "id, flow_id, flow_name, flow_user, flow_status)VALUES('N', "
            "'2020-01-08 08:33:42.014', '2020-01-14 07:04:50.432', 5, "
            "'2b40b793-0414-4e1b-8704-1314c093394b', 'JGSSデータ登録', 1, 'A');")
