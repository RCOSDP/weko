...
BEGIN
    IF (SELECT COUNT(id) FROM authors_prefix_settings WHERE scheme='e-Rad')>0 THEN
        UPDATE authors_prefix_settings SET scheme='Rad_Researcher',url='' WHERE scheme='e-Rad';
    END IF;
    IF (SELECT COUNT(id) FROM authors_prefix_settings WHERE scheme='e-Rad')=0 THEN
        INSERT INTO authors_prefix_settings ( name,scheme,url,created,updated ) VALUES ('Rad_Researcher','Rad_Researcher','','2024-12-24 00:00:00','2024-12-24 00:00:00');
    END IF;
