    ...
    if add_to_schema is not None \
        ...
        try:
            ...
            print(
                ...
                f'Successfully modified {item_type.item_type_name.name}'
            )
            ...
            print(
                datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S'),
                err,
                f'Failed to modify {item_type.item_type_name.name}'
            )
...
if creator_item_key:
    ...
    item_type_full[0].render["table_row_map"]["schema"]["properties"][creator_item_key[0]] = add_to_schema_creator_name_type["creatorNameType"]
...
if len(contributor_property.schema.get("properties", {})) > 0:
    for schema_prop_key in list(contributor_property.schema["properties"].keys()):
        if len(contributor_property.schema["properties"][schema_prop_key].get("items", {})) > 0:
            if len(contributor_property.schema["properties"][schema_prop_key]["items"].get("properties", {})) > 0:
                if len(contributor_property.schema["properties"][schema_prop_key]["items"]["properties"].get("contributorName", {})) > 0:
                    contributor_name_property_schema_property = contributor_property.schema["properties"][schema_prop_key]["items"]["properties"]

#? Get Contributor Name form from Contributor Property
contributor_name_property_form: dict or None = None
if len(contributor_property.form.get("items")) > 0:
    for contributor_prop_form in contributor_property.form["items"]:
...
if len(contributor_property.forms.get("items")) > 0:
    for contributor_prop_form in contributor_property.forms["items"]:
        if contributor_prop_form.get("items") is not None:
            for contributor_prop_form_sublevel in contributor_prop_form.get("items"):
                if "contributorNames" in contributor_prop_form_sublevel["key"]:
...
if contributor_item_key:
    ...
    item_type_full[0].render["table_row_map"]["schema"]["properties"][contributor_item_key[0]] = add_to_schema_contributor_name_type["contributorNameType"]
    ...
    if len(contributor_name_property_schema_property) > 0:
    ...
    try:
        ...
        with db.session.begin_nested():
            ...
            flag_modified(item_type_full[0], "form")
            flag_modified(item_type_full[0], "render")
            flag_modified(contributor_property, "schema")
            flag_modified(contributor_property, "form")
            flag_modified(contributor_property, "forms")
            for mapping in item_type_full_mapping:
                flag_modified(mapping, "mapping")
                db.session.merge(mapping)
            db.session.merge(item_type_full[0])
            db.session.merge(contributor_property)
        db.session.commit()
        print(
            datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S'),
...
funding_reference_property: ItemTypeProperty or None = None
for prop in all_item_type_properties:
     schema = prop.schema.get("properties", {})
     schema_keys = list(schema.keys())
     for funding_reference_schema_key in schema_keys:
        if schema.get(funding_reference_schema_key, {}).get('title'):
            if "助成機関" in schema.get(funding_reference_schema_key, {}).get('title'):
                funding_reference_property = prop

#? Get Funder Identifier schema from Funding Reference Property
funder_identifier_property_schema_property: dict or None = None
if len(funding_reference_property.schema.get("properties", {})) > 0:
    for schema_prop_key in list(funding_reference_property.schema["properties"].keys()):
...
if funding_reference_property.form.get("items"):
    for funding_reference_render_form in funding_reference_property.form["items"]:
        if funding_reference_render_form.get("items"):
            if isinstance(funding_reference_render_form["items"], list):
                if funding_reference_render_form["items"]:
...
if funding_reference_property.forms.get("items"):
    for funding_reference_render_form in funding_reference_property.forms["items"]:
        if funding_reference_render_form.get("items"):
            if isinstance(funding_reference_render_form["items"], list):
                if funding_reference_render_form["items"]:
                    if funding_reference_render_form["items"][0].get("title"):
...
if funding_reference_key:
    ...
    item_type_full[0].render["table_row_map"]["schema"]["properties"][funding_reference_key] = add_to_schema_funder_identifier_type_uri["funderIdentifierTypeURI"]
    ...
    if len(funder_identifier_property_schema_property) > 0:
    ...
    try:
        ...
        with db.session.begin_nested():
            ...
            flag_modified(item_type_full[0], "form")
            flag_modified(item_type_full[0], "render")
            flag_modified(funding_reference_property, "schema")
            flag_modified(funding_reference_property, "form")
            flag_modified(funding_reference_property, "forms")
            for mapping in item_type_full_mapping:
                flag_modified(mapping, "mapping")
                db.session.merge(mapping)
            db.session.merge(item_type_full[0])
            db.session.merge(funding_reference_property)
        db.session.commit()
        print(
            datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S'),
...
if len(funding_reference_property.schema.get("properties", {})) > 0:
    for schema_prop_key in list(funding_reference_property.schema["properties"].keys()):
        if len(funding_reference_property.schema["properties"][schema_prop_key].get("properties", {})) > 0:
            for schema_prop_sub_lv_key in list(funding_reference_property.schema["properties"][schema_prop_key].get("properties", {}).keys()):
                if funding_reference_property.schema["properties"][schema_prop_key]["properties"][schema_prop_sub_lv_key].get("title"):
                    if funding_reference_property.schema["properties"][schema_prop_key]["properties"][schema_prop_sub_lv_key]["title"] == "研究課題番号":
                        award_number_property_schema_property = funding_reference_property.schema["properties"][schema_prop_key]["properties"]
...
if funding_reference_key:
    ...
    item_type_full[0].render["table_row_map"]["schema"]["properties"][funding_reference_key] = add_to_schema_award_number_type["awardNumberType"]
    ...
    if len(award_number_property_schema_property) > 0:
...
subject_property: ItemTypeProperty or None = None
for prop in all_item_type_properties:
     schema = prop.schema.get("properties", {})
     schema_keys = list(schema.keys())
...
if len(subject_property.schema.get("properties", {})) > 0:
    for schema_prop_key in list(subject_property.schema["properties"].keys()):
        if len(subject_property.schema["properties"][schema_prop_key].get("enum", {})) > 0:
            if "主題Scheme" in subject_property.schema["properties"][schema_prop_key]["title"] \
                    or "Scheme" in subject_property.schema["properties"][schema_prop_key]["title"]:
                subject_scheme_property_schema_property = subject_property.schema["properties"][schema_prop_key]

#? Get Subject scheme form from Subject Property
subject_scheme_property_form: dict or None = None
if len(subject_property.form.get("items")) > 0:
    for subj_scheme_form in subject_property.form["items"]:
...
if len(subject_property.forms.get("items")) > 0:
    for subj_scheme_forms in subject_property.forms["items"]:
...
if subject_item_key:
    ...
    try:
        ...
        with db.session.begin_nested():
            ...
            flag_modified(item_type_full[0], "form")
            flag_modified(item_type_full[0], "render")
            flag_modified(subject_property, "schema")
            flag_modified(subject_property, "form")
            flag_modified(subject_property, "forms")
            db.session.merge(item_type_full[0])
            db.session.merge(subject_property)
        db.session.commit()
        print(
            datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S'),
...
if len(contributor_property.schema.get("properties", {})) > 0:
    for schema_prop_key in list(contributor_property.schema["properties"].keys()):
        if len(contributor_property.schema["properties"][schema_prop_key].get("items", {})) > 0:
            if len(contributor_property.schema["properties"][schema_prop_key]["items"].get("properties", {})) > 0:
                if len(contributor_property.schema["properties"][schema_prop_key]["items"]["properties"].get("contributorAffiliationNameIdentifiers", {})) > 0:
                    contributor_affiliation_property_schema_property = contributor_property.schema["properties"][schema_prop_key]["items"]["properties"]
                        ...
                        for fund_id_type_forms_sub_lv2 in fund_id_type_forms_sub_lv1["items"]:
                            if "Funder Identifier Type" in  fund_id_type_forms_sub_lv2.get("title_i18n", {}).get("en", {}) \
                                ...
                                if fund_id_type_forms_sub_lv2.get("titleMap") \
                                    ...
                                    print(fund_id_type_forms_sub_lv2)
