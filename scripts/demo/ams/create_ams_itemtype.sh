...
docker cp scripts/demo/ams/item_type_property.sql $(docker-compose -f docker-compose2.yml ps -q postgresql):/tmp/ams_item_type_property.sql
...
docker-compose -f docker-compose2.yml exec postgresql psql -U invenio -d invenio -f /tmp/ams_item_type_property.sql
