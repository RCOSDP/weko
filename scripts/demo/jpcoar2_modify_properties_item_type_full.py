...
#* Terminal command
#* sudo docker compose exec web invenio shell scripts/demo/jpcoar2_modify_properties_item_type_full.py

#* Get Full Item Type as well as Full Item Type Mapping
...
item_type_full_mapping_keys: [str] or None = None
if len(item_type_full_mapping) > 0:
    item_type_full_mapping_keys = [key for key in item_type_full_mapping[0].mapping.keys()]

#* All existing properties
all_item_type_properties = ItemTypeProps.get_records([])
schema_keys = list(item_type_full[0].schema["properties"].keys())

#* ========================================== CREATOR TYPE
def update_creator_for_creator_type_only(
        ...
        # if False:
            ...
            for mapping in item_type_mapping:
                if mapping.mapping.get(item_key) is not None:
                    if mapping.mapping[item_key].get("jpcoar_mapping"):
                        if mapping.mapping[item_key]["jpcoar_mapping"].get("creator"):
                            mapping.mapping[item_key]["jpcoar_mapping"]["creator"]["@attributes"] = {mapping_key: mapping_key}
...
creator_item_key: [str] = [
    creator
    for creator
    in list(item_type_full[0].schema["properties"].keys())
    if item_type_full[0].schema["properties"][creator].get("items", {}).get("properties", {}).get("creatorAffiliations")
    or item_type_full[0].schema["properties"][creator].get("properties", {}).get("creatorAffiliations")
]
...
if creator_item_key:
    update_creator_for_creator_type_only(
...
if item_type_full[0].schema["properties"][creator_item_key[0]] \
            ["items"]["properties"]["creatorNames"]["items"]["properties"].get("creatorNameType"):
    print("CREATOR NAME TYPE already updated for デフォルトアイテムタイプ（フル）.")
else:
    if creator_item_key:
        #* Modify ITEM TYPE Full for Creator Name Type
        if item_type_full[0].schema["properties"][creator_item_key[0]].get("items", {}).get("properties", {}).get("creatorNames", {}).get("items", {}).get("properties"):
            item_type_full[0].schema["properties"][creator_item_key[0]] \
                ["items"]["properties"]["creatorNames"]["items"]["properties"]["creatorNameType"] = add_to_schema_creator_name_type["creatorNameType"]
        creator_name_form.append(add_to_form_creator_name_type)
        if item_type_full[0].render["schemaeditor"]["schema"][creator_item_key[0]].get("properties", {}).get("creatorNames", {}).get("items", {}).get("properties"):
            item_type_full[0].render["schemaeditor"]["schema"][creator_item_key[0]] \
                ["properties"]["creatorNames"]["items"]["properties"]["creatorNameType"] = add_to_schema_creator_name_type["creatorNameType"]
        if item_type_full[0].render["table_row_map"]["schema"]["properties"][creator_item_key[0]] \
                .get("items", {}).get("properties", {}).get("creatorNames", {}).get("items", {}).get("properties"):
            item_type_full[0].render["table_row_map"]["schema"]["properties"][creator_item_key[0]] \
                ["items"]["properties"]["creatorNames"]["items"]["properties"]["creatorNameType"] = add_to_schema_creator_name_type["creatorNameType"]
        creator_name_render_form[0]["items"].append(add_to_form_creator_name_type)
        if len(item_type_full[0].render["table_row_map"]["mapping"][creator_item_key[0]]["jpcoar_mapping"].get("creator")) > 0:
            if len(item_type_full[0].render["table_row_map"]["mapping"][creator_item_key[0]]["jpcoar_mapping"]["creator"].get("creatorName")) > 0:
                if len(item_type_full[0].render["table_row_map"]["mapping"][creator_item_key[0]]["jpcoar_mapping"]["creator"]["creatorName"].get("@attributes")) > 0:
                    item_type_full[0].render["table_row_map"]["mapping"][creator_item_key[0]]["jpcoar_mapping"]["creator"]["creatorName"]["@attributes"]["nameType"] = "creatorNames.creatorNameType"
        ...
        #* Update target item type using update() method from ItemTypes
        ItemTypes.update(
            id_=item_type_full[0].id,
            name=item_type_full[0].item_type_name.name,
            schema=item_type_full[0].schema,
            form=item_type_full[0].form,
            render=item_type_full[0].render,
        )

        try:
            #* Save changes to DB
            with db.session.begin_nested():
                flag_modified(item_type_full[0], "schema")
                flag_modified(item_type_full[0], "form")
                flag_modified(item_type_full[0], "render")
                flag_modified(creator_property, "schema")
                flag_modified(creator_property, "form")
                flag_modified(creator_property, "forms")
                for mapping in item_type_full_mapping:
                    flag_modified(mapping, "mapping")
                    db.session.merge(mapping)
                db.session.merge(item_type_full[0])
                db.session.merge(creator_property)
            db.session.commit()
            print(
                datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S'),
                f'Successfully modified {item_type_full[0].item_type_name.name} for CREATOR NAME TYPE'
            )
...
contributor_item_key: [str] = [
    ...
    for contributor
    in list(item_type_full[0].schema["properties"].keys())
    if item_type_full[0].schema["properties"][contributor].get("items", {}).get("properties", {}).get("contributorAffiliations")
    or item_type_full[0].schema["properties"][contributor].get("properties", {}).get("contributorAffiliations")
]
...
if item_type_full[0].schema["properties"][contributor_item_key[0]] \
        ["items"]["properties"]["contributorNames"]["items"]["properties"].get("contributorNameType"):
    print("CONTRIBUTOR NAME TYPE already updated for デフォルトアイテムタイプ（フル）.")
else:
    if contributor_item_key:
        #* Modify ITEM TYPE Full for Contributor Name Type
        if item_type_full[0].schema["properties"][contributor_item_key[0]] \
                .get("items", {}).get("properties", {}).get("contributorNames", {}).get("items", {}).get("properties"):
            item_type_full[0].schema["properties"][contributor_item_key[0]] \
                    ["items"]["properties"]["contributorNames"]["items"]["properties"]["contributorNameType"] = add_to_schema_contributor_name_type["contributorNameType"]
        
        contributor_name_form.append(add_to_form_contributor_name_type)

        if item_type_full[0].render["schemaeditor"]["schema"][contributor_item_key[0]] \
                .get("properties", {}).get("contributorNames", {}).get("items", {}).get("properties"):
            item_type_full[0].render["schemaeditor"]["schema"][contributor_item_key[0]] \
                ["properties"]["contributorNames"]["items"]["properties"]["contributorNameType"] = add_to_schema_contributor_name_type["contributorNameType"]
            
        if item_type_full[0].render["table_row_map"]["schema"]["properties"][contributor_item_key[0]] \
                .get("items", {}).get("properties", {}).get("contributorNames", {}).get("items", {}).get("properties"):
            item_type_full[0].render["table_row_map"]["schema"]["properties"][contributor_item_key[0]] \
                ["items"]["properties"]["contributorNames"]["items"]["properties"]["contributorNameType"] = add_to_schema_contributor_name_type["contributorNameType"]
            
        contributor_name_render_form[0]["items"].append(add_to_form_contributor_name_type)

        if len(item_type_full[0].render["table_row_map"]["mapping"][contributor_item_key[0]]["jpcoar_mapping"].get("contributor")) > 0:
            if len(item_type_full[0].render["table_row_map"]["mapping"][contributor_item_key[0]]["jpcoar_mapping"]["contributor"].get("contributorName")) > 0:
                if len(item_type_full[0].render["table_row_map"]["mapping"][contributor_item_key[0]]["jpcoar_mapping"]["contributor"]["contributorName"].get("@attributes")) > 0:
                    item_type_full[0].render["table_row_map"]["mapping"][contributor_item_key[0]]["jpcoar_mapping"]["contributor"]["contributorName"]["@attributes"]["nameType"] = "contributorNames.creatorNameType"
        ...
        try:
            ...
            print(
                datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S'),
                f'Successfully modified {item_type_full[0].item_type_name.name} for CONTRIBUTOR NAME TYPE'
            )
...
for fund_ref_key in list(item_type_full[0].schema["properties"].keys()):
    if item_type_full[0].schema["properties"][fund_ref_key].get("items"):
        if item_type_full[0].schema["properties"][fund_ref_key]["items"].get("properties", {}):
            for prop_key in list(item_type_full[0].schema["properties"][fund_ref_key]["items"]["properties"].keys()):
                if item_type_full[0].schema["properties"][fund_ref_key]["items"]["properties"][prop_key].get("title"):
                    if item_type_full[0].schema["properties"][fund_ref_key]["items"]["properties"][prop_key]["title"] == "助成機関識別子" \
                            or item_type_full[0].schema["properties"][fund_ref_key]["items"]["properties"][prop_key]["title"] == "助成機関名" \
                            or item_type_full[0].schema["properties"][fund_ref_key]["items"]["properties"][prop_key]["title"] == "研究課題番号" \
                            or item_type_full[0].schema["properties"][fund_ref_key]["items"]["properties"][prop_key]["title"] == "研究課題名":
...
for funding_reference_form in item_type_full[0].form:
...
for funding_reference_render_form in item_type_full[0].render["table_row_map"]["form"]:
...
if item_type_full[0].schema["properties"][funding_reference_key].get("items", {}).get("properties"):
    if isinstance(item_type_full[0].schema["properties"][funding_reference_key].get("items", {}).get("properties"), dict):
        for fund_id_type_uri_keys in item_type_full[0].schema["properties"][funding_reference_key]["items"]["properties"].keys():
            if "助成機関識別子" in item_type_full[0].schema["properties"][funding_reference_key]["items"]["properties"].get(fund_id_type_uri_keys, {}).get("title"):
                if item_type_full[0].schema["properties"][funding_reference_key]["items"]["properties"] \
                        [fund_id_type_uri_keys]["properties"].get("funderIdentifierTypeURI"):
                    print("FUNDING IDENTIFIER TYPE URI already updated for デフォルトアイテムタイプ（フル）.")
else:
    if funding_reference_key:
        ...
        if item_type_full[0].schema["properties"][funding_reference_key].get("items", {}).get("properties"):
            if isinstance(item_type_full[0].schema["properties"][funding_reference_key].get("items", {}).get("properties"), dict):
                for fund_id_type_uri_keys in item_type_full[0].schema["properties"][funding_reference_key]["items"]["properties"].keys():
                    if "助成機関識別子" in item_type_full[0].schema["properties"][funding_reference_key]["items"]["properties"].get(fund_id_type_uri_keys, {}).get("title"):
                        item_type_full[0].schema["properties"][funding_reference_key]["items"]["properties"] \
                            [fund_id_type_uri_keys]["properties"]["funderIdentifierTypeURI"] = add_to_schema_funder_identifier_type_uri["funderIdentifierTypeURI"]

        funding_reference_funder_identifier_form_list.append(add_to_form_funder_identifier_type_uri)

        if item_type_full[0].render["schemaeditor"]["schema"][funding_reference_key].get("properties"):
            if isinstance(item_type_full[0].render["schemaeditor"]["schema"][funding_reference_key].get("properties"), dict):
                for fund_id_type_uri_keys in item_type_full[0].render["schemaeditor"]["schema"][funding_reference_key]["properties"].keys():
                    if "助成機関識別子" in item_type_full[0].render["schemaeditor"]["schema"] \
                                [funding_reference_key]["properties"][fund_id_type_uri_keys].get("title"):
                        item_type_full[0].render["schemaeditor"]["schema"][funding_reference_key]["properties"][fund_id_type_uri_keys] \
                            ["properties"]["funderIdentifierTypeURI"] = add_to_schema_funder_identifier_type_uri["funderIdentifierTypeURI"]

        if item_type_full[0].render["table_row_map"]["schema"]["properties"][funding_reference_key].get("items", {}).get("properties"):
            if isinstance(item_type_full[0].render["table_row_map"]["schema"]["properties"][funding_reference_key].get("items", {}).get("properties"), dict):
                for fund_id_type_uri_keys in item_type_full[0].render["table_row_map"]["schema"]["properties"][funding_reference_key]["items"]["properties"].keys():
                    if "助成機関識別子" in item_type_full[0].render["table_row_map"]["schema"]["properties"][funding_reference_key]["items"]["properties"].get(fund_id_type_uri_keys, {}).get("title"):
                        item_type_full[0].render["table_row_map"]["schema"]["properties"][funding_reference_key]["items"]["properties"] \
                            [fund_id_type_uri_keys]["properties"]["funderIdentifierTypeURI"] = add_to_schema_funder_identifier_type_uri["funderIdentifierTypeURI"]

        funding_reference_render_form_list.append(add_to_form_funder_identifier_type_uri)
        if len(item_type_full[0].render["table_row_map"]["mapping"][funding_reference_key]["jpcoar_mapping"].get("fundingReference")) > 0:
            if len(item_type_full[0].render["table_row_map"]["mapping"][funding_reference_key]["jpcoar_mapping"]["fundingReference"].get("funderIdentifier")) > 0:
                if len(item_type_full[0].render["table_row_map"]["mapping"][funding_reference_key]["jpcoar_mapping"]["fundingReference"]["funderIdentifier"].get("@attributes")) > 0:
                    item_type_full[0].render["table_row_map"]["mapping"][funding_reference_key]["jpcoar_mapping"]["fundingReference"]["funderIdentifier"]["@attributes"]["funderIdentifierTypeURI"] = f"{funder_identifier_type_key_mapping_use}.funderIdentifierTypeURI"
        ...
        try:
            ...
            print(
                datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S'),
                f'Successfully modified {item_type_full[0].item_type_name.name} for FUNDING IDENTIFIER TYPE URI'
            )
...
if award_number_type:
    ...
    if funding_reference_key:
        ...
        if item_type_full[0].schema["properties"][funding_reference_key].get("items", {}).get("properties"):
            if isinstance(item_type_full[0].schema["properties"][funding_reference_key].get("items", {}).get("properties"), dict):
                for fund_id_type_uri_keys in item_type_full[0].schema["properties"][funding_reference_key]["items"]["properties"].keys():
                    if "研究課題番号" in item_type_full[0].schema["properties"][funding_reference_key]["items"]["properties"].get(fund_id_type_uri_keys, {}).get("title"):
                        item_type_full[0].schema["properties"][funding_reference_key]["items"]["properties"] \
                            [fund_id_type_uri_keys]["properties"]["awardNumberType"] = add_to_schema_award_number_type["awardNumberType"]
        ...
        if item_type_full[0].render["schemaeditor"]["schema"][funding_reference_key].get("properties"):
            if isinstance(item_type_full[0].render["schemaeditor"]["schema"][funding_reference_key].get("properties"), dict):
                for fund_id_type_uri_keys in item_type_full[0].render["schemaeditor"]["schema"][funding_reference_key]["properties"].keys():
                    if "研究課題番号" in item_type_full[0].render["schemaeditor"]["schema"] \
                                [funding_reference_key]["properties"][fund_id_type_uri_keys].get("title"):
                        item_type_full[0].render["schemaeditor"]["schema"][funding_reference_key]["properties"][fund_id_type_uri_keys] \
                            ["properties"]["awardNumberType"] = add_to_schema_award_number_type["awardNumberType"]

        if item_type_full[0].render["table_row_map"]["schema"]["properties"][funding_reference_key].get("items", {}).get("properties"):
            if isinstance(item_type_full[0].render["table_row_map"]["schema"]["properties"][funding_reference_key].get("items", {}).get("properties"), dict):
                for fund_id_type_uri_keys in item_type_full[0].render["table_row_map"]["schema"]["properties"][funding_reference_key]["items"]["properties"].keys():
                    if "研究課題番号" in item_type_full[0].render["table_row_map"]["schema"]["properties"][funding_reference_key]["items"]["properties"].get(fund_id_type_uri_keys, {}).get("title"):
                        item_type_full[0].render["table_row_map"]["schema"]["properties"][funding_reference_key]["items"]["properties"] \
                            [fund_id_type_uri_keys]["properties"]["awardNumberType"] = add_to_schema_award_number_type["awardNumberType"]

        funding_reference_render_form_list.append(add_to_form_award_number_type)
        if len(item_type_full[0].render["table_row_map"]["mapping"][funding_reference_key]["jpcoar_mapping"].get("fundingReference")) > 0:
            if len(item_type_full[0].render["table_row_map"]["mapping"][funding_reference_key]["jpcoar_mapping"]["fundingReference"].get("awardNumber")) > 0:
                if len(item_type_full[0].render["table_row_map"]["mapping"][funding_reference_key]["jpcoar_mapping"]["fundingReference"]["awardNumber"].get("@attributes")) > 0:
                    item_type_full[0].render["table_row_map"]["mapping"][funding_reference_key]["jpcoar_mapping"]["fundingReference"]["awardNumber"]["@attributes"]["awardNumberType"] = f"{award_number_type_key_mapping_use}.awardNumberType"
        ...
        try:
            ...
            print(
                datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S'),
                f'Successfully modified {item_type_full[0].item_type_name.name} for AWARD NUMBER TYPE'
            )
...
for subject in list(item_type_full[0].schema["properties"].keys()):
    for y in list(item_type_full[0].schema["properties"][subject].get("items",{}).get("properties", {}).keys()):
        if item_type_full[0].schema["properties"][subject].get("items",{}).get("properties", {}).get(y, {}).get("title") or \
            item_type_full[0].schema["properties"][subject].get("items",{}).get("properties", {}).get(y, {}).get("title_i18n"):
            if "主題" in item_type_full[0].schema["properties"][subject].get("items",{}).get("properties", {}).get(y, {}).get("title") or \
                "Subject URI" in item_type_full[0].schema["properties"][subject].get("items",{}).get("properties", {}).get(y, {}).get("title_i18n", {}).get("en", {}) \
                    or "主題URI" in item_type_full[0].schema["properties"][subject].get("items",{}).get("properties", {}).get(y, {}).get("title_i18n", {}).get("ja", {}):
...
if subject_item_key:
    if item_type_full[0].schema["properties"][subject_item_key].get("items", {}).get("properties"):
        if isinstance(item_type_full[0].schema["properties"][subject_item_key]["items"]["properties"], dict):
            for subj_scheme_item in list(item_type_full[0].schema["properties"][subject_item_key]["items"]["properties"].keys()):
                if item_type_full[0].schema["properties"][subject_item_key]["items"]["properties"][subj_scheme_item].get("title"):
                    if "主題Scheme" in item_type_full[0].schema["properties"][subject_item_key]["items"]["properties"][subj_scheme_item]["title"] or \
                        "Scheme" in item_type_full[0].schema["properties"][subject_item_key]["items"]["properties"][subj_scheme_item]["title"]:
                        subject_scheme_schema = item_type_full[0].schema["properties"][subject_item_key]["items"]["properties"][subj_scheme_item]
...
if "JEL" in subject_scheme_schema.get("currentEnum",[]) \
    ...
    if subject_item_key:
        ...
        if not subject_scheme_schema.get("currentEnum") \
                and subject_scheme_schema.get("enum"):
            subject_scheme_schema["currentEnum"] = subject_scheme_schema["enum"]
            for sub_sch in subject_scheme_new_values:
                subject_scheme_schema["currentEnum"].append(sub_sch)
                subject_scheme_schema["enum"].append(sub_sch)
        ...
        try:
            ...
            print(
                datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S'),
                f'Successfully modified {item_type_full[0].item_type_name.name} for SUBJECT SCHEME'
            )
...
if creator_item_key:
    if item_type_full[0].schema["properties"][creator_item_key[0]] \
        ...
        creator_affiliation_name_identifier_schema = item_type_full[0].schema["properties"][creator_item_key[0]] \
            ["items"] \
            ["properties"] \
            ["creatorAffiliations"] \
            ["items"] \
...
if "kakenhi【非推奨】" in creator_affiliation_name_identifier_schema.get("currentEnum",[]) \
    ...
    print("CREATOR AFFILIATION NAME IDENTIFIER SCHEME already updated for デフォルトアイテムタイプ（フル）.")
else:
    if creator_item_key:
        ...
        if creator_affiliation_identifier_scheme_property_schema_property:
            if creator_affiliation_identifier_scheme_property_schema_property.get("enum"):
                if ror not in creator_affiliation_identifier_scheme_property_schema_property["enum"]:
        ...
        try:
            ...
            print(
                datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S'),
                f'Successfully modified {item_type_full[0].item_type_name.name} for CREATOR AFFILIATION NAME IDENTIFIER SCHEME'
            )
...
if contributor_item_key:
    if item_type_full[0].schema["properties"][contributor_item_key[0]] \
        ...
        contributor_affiliation_name_identifier_schema = item_type_full[0].schema["properties"][contributor_item_key[0]] \
            ["items"] \
            ["properties"] \
...
if "kakenhi【非推奨】" in contributor_affiliation_name_identifier_schema.get("currentEnum",[]) \
    ...
    print("CONTRIBUTOR AFFILIATION NAME IDENTIFIER SCHEME already updated for デフォルトアイテムタイプ（フル）.")
else:
    if contributor_item_key:
        ...
        if contributor_property:
            for contributor_prop in contributor_property:
                ...
                if contributor_affiliation_property_form:
                    for contrib_aff_prop_form in contributor_affiliation_property_form:
                        if contrib_aff_prop_form.get("items") and isinstance(contrib_aff_prop_form.get("items"), list):
                            for contrib_aff_prop_form_lv1 in contrib_aff_prop_form["items"]:
                                if contrib_aff_prop_form_lv1.get("key") \
                                    ...
                                    if contrib_aff_prop_form_lv1.get("titleMap"):
                                        if ror_title_map not in contrib_aff_prop_form_lv1["titleMap"]:
                ...
                if contributor_affiliation_property_forms:
                    for contrib_aff_prop_forms in contributor_affiliation_property_forms:
                        if contrib_aff_prop_forms.get("items") and isinstance(contrib_aff_prop_forms.get("items"), list):
                            for contrib_aff_prop_forms_lv1 in contrib_aff_prop_forms["items"]:
                                if contrib_aff_prop_forms_lv1.get("key") \
                                    ...
                                    if contrib_aff_prop_forms_lv1.get("titleMap"):
                                        if ror_title_map not in contrib_aff_prop_forms_lv1["titleMap"]:
        ...
        try:
            ...
            print(
                datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S'),
                f'Successfully modified {item_type_full[0].item_type_name.name} for CONTRIBUTOR AFFILIATION NAME IDENTIFIER SCHEME'
            )
...
for relation in list(item_type_full[0].schema["properties"].keys()):
    for rel_key in list(item_type_full[0].schema["properties"][relation].get("items",{}).get("properties", {}).keys()):
        if item_type_full[0].schema["properties"][relation] \
                .get("items",{}) \
                .get("properties", {}) \
                .get(rel_key, {}) \
                .get("items", {}) \
                .get("properties"):
            if isinstance(
                item_type_full[0].schema["properties"] \
                [relation] \
                ["items"] \
                ["properties"] \
                [rel_key] \
                ...
                for key in list(item_type_full[0].schema["properties"] \
                    [relation] \
                    ["items"] \
                    ["properties"] \
                    [rel_key] \
                    ...
                    or item_type_full[0].schema["properties"] \
                            [relation] \
                            ["items"] \
                            ["properties"] \
                            [rel_key] \
...
if relation_item_key:
    if item_type_full[0].schema["properties"][relation_item_key].get("items", {}).get("properties"):
        if isinstance(item_type_full[0].schema["properties"][relation_item_key]["items"]["properties"], dict):
            for rel_type_schema_key in list(item_type_full[0].schema["properties"][relation_item_key]["items"]["properties"].keys()):
                if "関連タイプ" in item_type_full[0].schema["properties"] \
                        [relation_item_key] \
                        ["items"] \
                        ["properties"] \
                        [rel_type_schema_key] \
                        .get("title", {}) \
                or "isVersionOf" in item_type_full[0].schema["properties"] \
                        [relation_item_key] \
                        ["items"] \
                        ["properties"] \
                        [rel_type_schema_key] \
                        .get("enum", {}):
                    relation_type_schema = item_type_full[0].schema["properties"][relation_item_key]["items"]["properties"][rel_type_schema_key]
...
if "inSeries" in relation_type_schema.get("currentEnum",[]) \
    ...
    if relation_item_key:
        ...
        try:
            ...
            print(
                datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S'),
                f'Successfully modified {item_type_full[0].item_type_name.name} for RELATION TYPE'
            )
            ...
            for rel_type_schema_key in list(item_type_full[0].schema["properties"][relation_item_key]["items"]["properties"].keys()):
                if item_type_full[0].schema["properties"] \
                    ...
                    if isinstance(
                        ...
                        for rel_id_key in list(item_type_full[0].schema["properties"] \
                                [relation_item_key] \
                                ["items"] \
                                ["properties"] \
                                [rel_type_schema_key] \
                                .get("properties").keys()):
                            if "arXiv" in item_type_full[0].schema["properties"] \
                                    [relation_item_key] \
                                    ["items"] \
                                    ["properties"] \
                                    [rel_type_schema_key] \
                                    ...
                                    [rel_id_key] \
                                    .get("enum", {}) \
                            or "識別子タイプ" in item_type_full[0].schema["properties"] \
                                    [relation_item_key] \
                                    ["items"] \
                                    ["properties"] \
                                    [rel_type_schema_key] \
...
if "CRID" in relation_identifier_type_schema.get("currentEnum",[]) \
    ...
    if relation_item_key:
        ...
        try:
            ...
            print(
                datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S'),
                f'Successfully modified {item_type_full[0].item_type_name.name} for RELATION IDENTIFIER TYPE'
            )
...
if funding_reference_key:
    if item_type_full[0].schema["properties"][funding_reference_key].get("items", {}).get("properties"):
        if isinstance(item_type_full[0].schema["properties"][funding_reference_key]["items"]["properties"], dict):
            for fund_id_type_schema_key in list(item_type_full[0].schema["properties"][funding_reference_key]["items"]["properties"].keys()):
                if item_type_full[0].schema["properties"][funding_reference_key]["items"]["properties"][fund_id_type_schema_key].get("properties"):
                    if isinstance(
                        item_type_full[0].schema["properties"][funding_reference_key]["items"]["properties"][fund_id_type_schema_key].get("properties"),
                        dict
                    ):
                        for fund_id_type_key \
                        in list(item_type_full[0].schema["properties"][funding_reference_key]["items"]["properties"][fund_id_type_schema_key].get("properties").keys()):
                            if (item_type_full[0].schema["properties"] \
                            ...
                            and ("助成機関識別子タイプ" in item_type_full[0].schema["properties"] \
                                    [funding_reference_key]["items"]["properties"][fund_id_type_schema_key]["properties"][fund_id_type_key].get("title") \
                                or "識別子タイプ" in item_type_full[0].schema["properties"] \
                                    [funding_reference_key]["items"]["properties"][fund_id_type_schema_key]["properties"][fund_id_type_key].get("title")):
                                funder_identifier_type_schema = item_type_full[0].schema["properties"] \
...
if "ROR" in funder_identifier_type_schema.get("currentEnum",[]) \
    ...
    if funding_reference_key:
        ...
        try:
            ...
            print(
                datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S'),
                f'Successfully modified {item_type_full[0].item_type_name.name} for FUNDER IDENTIFIER TYPE'
            )
