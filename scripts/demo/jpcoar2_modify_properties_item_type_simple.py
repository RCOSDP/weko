...
if item_type_simple[0].schema["properties"][creator_item_key[0]] \
    ...
    print("CREATOR NAME TYPE already updated for デフォルトアイテムタイプ（シンプル）.")
else:
    if creator_item_key:
        #* Modify ITEM TYPE Simple for Creator Name Type
        if item_type_simple[0].schema["properties"][creator_item_key[0]].get("items", {}).get("properties", {}).get("creatorNames", {}).get("items", {}).get("properties"):
            item_type_simple[0].schema["properties"][creator_item_key[0]] \
        ...
        if item_type_simple[0].render["schemaeditor"]["schema"][creator_item_key[0]].get("properties", {}).get("creatorNames", {}).get("items", {}).get("properties"):
            item_type_simple[0].render["schemaeditor"]["schema"][creator_item_key[0]] \
                ["properties"]["creatorNames"]["items"]["properties"]["creatorNameType"] = add_to_schema_creator_name_type["creatorNameType"]
        if item_type_simple[0].render["table_row_map"]["schema"]["properties"][creator_item_key[0]] \
                .get("items", {}).get("properties", {}).get("creatorNames", {}).get("items", {}).get("properties"):
            item_type_simple[0].render["table_row_map"]["schema"]["properties"][creator_item_key[0]] \
                ["items"]["properties"]["creatorNames"]["items"]["properties"]["creatorNameType"] = add_to_schema_creator_name_type["creatorNameType"]
...
if item_type_simple[0].schema["properties"][contributor_item_key[0]] \
    ...
    print("CONTRIBUTOR NAME TYPE already updated for デフォルトアイテムタイプ（シンプル）.")
else:
    if contributor_item_key:
        ...
        if item_type_simple[0].schema["properties"][contributor_item_key[0]] \
            ...
            item_type_simple[0].schema["properties"][contributor_item_key[0]] \
        ...
        if item_type_simple[0].render["schemaeditor"]["schema"][contributor_item_key[0]] \
                .get("properties", {}).get("contributorNames", {}).get("items", {}).get("properties"):
            item_type_simple[0].render["schemaeditor"]["schema"][contributor_item_key[0]] \
                ["properties"]["contributorNames"]["items"]["properties"]["contributorNameType"] = add_to_schema_contributor_name_type["contributorNameType"]
            
        if item_type_simple[0].render["table_row_map"]["schema"]["properties"][contributor_item_key[0]] \
                .get("items", {}).get("properties", {}).get("contributorNames", {}).get("items", {}).get("properties"):
            item_type_simple[0].render["table_row_map"]["schema"]["properties"][contributor_item_key[0]] \
                ["items"]["properties"]["contributorNames"]["items"]["properties"]["contributorNameType"] = add_to_schema_contributor_name_type["contributorNameType"]
...
if item_type_simple[0].schema["properties"][funding_reference_key].get("items", {}).get("properties"):
    if isinstance(item_type_simple[0].schema["properties"][funding_reference_key].get("items", {}).get("properties"), dict):
        for fund_id_type_uri_keys in item_type_simple[0].schema["properties"][funding_reference_key]["items"]["properties"].keys():
            if "助成機関識別子" in item_type_simple[0].schema["properties"][funding_reference_key]["items"]["properties"].get(fund_id_type_uri_keys, {}).get("title"):
                if item_type_simple[0].schema["properties"][funding_reference_key]["items"]["properties"] \
                        [fund_id_type_uri_keys]["properties"].get("funderIdentifierTypeURI"):
                    print("FUNDING IDENTIFIER TYPE URI already updated for デフォルトアイテムタイプ（シンプル）.")
else:
    if funding_reference_key:
        ...
        if item_type_simple[0].schema["properties"][funding_reference_key].get("items", {}).get("properties"):
            if isinstance(item_type_simple[0].schema["properties"][funding_reference_key].get("items", {}).get("properties"), dict):
                for fund_id_type_uri_keys in item_type_simple[0].schema["properties"][funding_reference_key]["items"]["properties"].keys():
                    if "助成機関識別子" in item_type_simple[0].schema["properties"][funding_reference_key]["items"]["properties"].get(fund_id_type_uri_keys, {}).get("title"):
                        item_type_simple[0].schema["properties"][funding_reference_key]["items"]["properties"] \
        ...
        if item_type_simple[0].render["schemaeditor"]["schema"][funding_reference_key].get("properties"):
            if isinstance(item_type_simple[0].render["schemaeditor"]["schema"][funding_reference_key].get("properties"), dict):
                for fund_id_type_uri_keys in item_type_simple[0].render["schemaeditor"]["schema"][funding_reference_key]["properties"].keys():
                    if "助成機関識別子" in item_type_simple[0].render["schemaeditor"]["schema"] \
                                [funding_reference_key]["properties"][fund_id_type_uri_keys].get("title"):
                        item_type_simple[0].render["schemaeditor"]["schema"][funding_reference_key]["properties"][fund_id_type_uri_keys] \
                            ["properties"]["funderIdentifierTypeURI"] = add_to_schema_funder_identifier_type_uri["funderIdentifierTypeURI"]

        if item_type_simple[0].render["table_row_map"]["schema"]["properties"][funding_reference_key].get("items", {}).get("properties"):
            if isinstance(item_type_simple[0].render["table_row_map"]["schema"]["properties"][funding_reference_key].get("items", {}).get("properties"), dict):
                for fund_id_type_uri_keys in item_type_simple[0].render["table_row_map"]["schema"]["properties"][funding_reference_key]["items"]["properties"].keys():
                    if "助成機関識別子" in item_type_simple[0].render["table_row_map"]["schema"]["properties"][funding_reference_key]["items"]["properties"].get(fund_id_type_uri_keys, {}).get("title"):
                        item_type_simple[0].render["table_row_map"]["schema"]["properties"][funding_reference_key]["items"]["properties"] \
                            [fund_id_type_uri_keys]["properties"]["funderIdentifierTypeURI"] = add_to_schema_funder_identifier_type_uri["funderIdentifierTypeURI"]
...
if item_type_simple[0].schema["properties"][funding_reference_key].get("items", {}).get("properties"):
    if isinstance(item_type_simple[0].schema["properties"][funding_reference_key].get("items", {}).get("properties"), dict):
        for fund_id_type_uri_keys in item_type_simple[0].schema["properties"][funding_reference_key]["items"]["properties"].keys():
            if "研究課題番号" in item_type_simple[0].schema["properties"][funding_reference_key]["items"]["properties"].get(fund_id_type_uri_keys, {}).get("title"):
                if item_type_simple[0].schema["properties"][funding_reference_key]["items"]["properties"] \
                        [fund_id_type_uri_keys]["properties"].get("awardNumberType"):
                    award_number_type = True

if award_number_type:
    print("AWARD NUMBER TYPE already updated for デフォルトアイテムタイプ（シンプル）.")
    ...
    if funding_reference_key:
        ...
        if item_type_simple[0].render["schemaeditor"]["schema"][funding_reference_key].get("properties"):
            if isinstance(item_type_simple[0].render["schemaeditor"]["schema"][funding_reference_key].get("properties"), dict):
                for fund_id_type_uri_keys in item_type_simple[0].render["schemaeditor"]["schema"][funding_reference_key]["properties"].keys():
                    if "研究課題番号" in item_type_simple[0].render["schemaeditor"]["schema"] \
                                [funding_reference_key]["properties"][fund_id_type_uri_keys].get("title"):
                        item_type_simple[0].render["schemaeditor"]["schema"][funding_reference_key]["properties"][fund_id_type_uri_keys] \
        ...
        if item_type_simple[0].render["table_row_map"]["schema"]["properties"][funding_reference_key].get("items", {}).get("properties"):
            if isinstance(item_type_simple[0].render["table_row_map"]["schema"]["properties"][funding_reference_key].get("items", {}).get("properties"), dict):
                for fund_id_type_uri_keys in item_type_simple[0].render["table_row_map"]["schema"]["properties"][funding_reference_key]["items"]["properties"].keys():
                    if "研究課題番号" in item_type_simple[0].render["table_row_map"]["schema"]["properties"][funding_reference_key]["items"]["properties"].get(fund_id_type_uri_keys, {}).get("title"):
                        item_type_simple[0].render["table_row_map"]["schema"]["properties"][funding_reference_key]["items"]["properties"] \
                            [fund_id_type_uri_keys]["properties"]["awardNumberType"] = add_to_schema_award_number_type["awardNumberType"]
                    ...
                    if isinstance(
                        ...
                        for rel_id_key in list(item_type_simple[0].schema["properties"] \
                            ...
                            or "識別子タイプ" in item_type_simple[0].schema["properties"] \
                                    ...
                                    [rel_id_key] \
                                    .get("title"):
                                if isinstance(item_type_simple[0].schema["properties"] \
                                        [relation_item_key]["items"]["properties"][rel_type_schema_key]["properties"], dict):
                                    for rel_id_type_schema_bottom_level in item_type_simple[0].schema["properties"] \
