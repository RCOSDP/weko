...
__plugin__ = "NewPlugin"
__version__ = "1.0.0"


def hello_world():
    flash("Hello New Plugin from {} Plugin".format(__plugin__), "success")


def hello_world2():
    flash("Hello New Plugin 2 from {} Plugin".format(__plugin__), "success")


def inject_hello_world():
    return "<h1>Hello New Plugin Injected</h1>"


def inject_hello_world2():
    return "<h1>Hello New Plugin 2 Injected</h1>"


def inject_navigation_link():
    return render_template_string(
        """
            <li><a href="{{ url_for('newplugin.index') }}">New Plugin</a></li>
        """)


hello = Blueprint("newplugin", __name__, template_folder="templates")


@hello.route("/")
def index():
    return render_template("hello.html", plugin_name='new_plugin')
