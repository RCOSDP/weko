...
**未修正のものは、チケット番号も書かない。** 番号から Redmine を引かれる。
修正が入ったリリースが出た後であれば、`[issue62569]` のように public 側にも書いてよい。
...
| 語 | 意味 |
|---|---|
| **台帳** | `weko3_api_list_full.tsv`(63列) / `weko3_api_list.tsv`(32列)。API の棚卸し結果。列定義は `scripts/schema.py` が唯一の正 |
