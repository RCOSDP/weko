...
> 2026-09-11 に手順を `docs/OPERATIONS.md` へ分離した（§9）。
> 2026-09-15 に §1 へ「判断するのはファイルではなく内容」を追加した（§9）。
> 2026-09-23 に §1 の「未修正はチケット番号も書かない」を改め、番号のみ可とした（§9）。

## 0. この文書の位置づけ

| 文書 | 書いてあること |
|---|---|
| **本書（`docs/RULE.md`）** | **守るべきルール。** 誰が・いつ・何をするか、なぜそうするか |
| `docs/OPERATIONS.md` | **作業手順。** PR の出し方・リリースの回し方を、上から実行できる形で |
...
| `tools/release/README.md` | PR とリリースの補助ツール（前提チェック・PR 作成） |

**本書に手順は書かない。** ここにあるのは「どうすべきか」を決める根拠だけで、
「どう打つか」は `docs/OPERATIONS.md` と各 README にある。
**迷ったら本書、手を動かすなら OPERATIONS.md。**
...
禁止しているのはファイルの commit だけではない。**台帳に書いてあることを public 側の
文章に書き写すのも、同じことをしている。** ツールのコード自体は public に置いてよいが、
その docstring・コメント・テストのフィクスチャ・コミットメッセージは文章であって、
そこに所見を書けば所見を公開したことになる。`.gitignore` も CI のゲートも、これは止めない。

> 実際に起きた例（2026-09-15、push 前に発見して巻き戻し）。認可の検出ツールを public 側に
> 追加したとき、「なぜこの検出が要るか」を説明するために、**未修正**の脆弱性の実関数
> チェーンと、欠けている検証の中身を docstring に書いた。テストのフィクスチャにも実在の
> 関数名・フィールド名をそのまま使った。ファイルは「ツール」なので §1 の表では public に
> 置いてよいものだったが、中身は台帳の `sec_detail` と同じだった。

#### 判定基準: その記述だけで攻撃が再現できるか

できるなら public に置かない。

| public 側に書いてよい | private 側にしか書かない |
|---|---|
| **どう探すか** — 検出ロジック、探索パターン、作業手順、環境の整え方 | **どこが該当するか** — 経路・関数・フィールド・パラメータの実名 |
| 「持っているだけで通る値で対象を引きながら、リクエストの識別子と照合しない実装は危ない」 | 「`<file>:<func>` が `<field>` を照合していない」 |
| 合成した最小コードのテストフィクスチャ | 実データ・実測値・該当件数の内訳 |

#### 未修正のチケット番号は、番号だけなら public に書いてよい

**番号だけなら分析を含まない。** 1000 経路の中から欠陥を見つける作業がこの台帳の
中身であって、番号はその作業を肩代わりしない。一方で、未修正を private 側の台帳に
しか書けないままにすると、**台帳を開く人が減った時点で忘れられる**。番号を出せる
ようにしておくほうが、忘れるより安全と判断した（§9、2026-09-23）。

条件は2つ。

1. **説明を添えない。** 「issue62810 ファイル配信の認可漏れ」と書いた時点で、
   それは番号ではなく所見である。経路名・関数名・症状のどれも添えない
2. **番号を脆弱な実装ファイルの近くに置かない。** 文言が無くても、番号が散って
   いれば `grep` で弱点の地図ができる。番号は**台帳の `fix_ticket` 列に集約**し、
   public には一覧としてだけ出す（`open_findings.py --summary-only`）

忘れないための仕掛けは CI 側に置く。`api-inventory-drift` が毎 PR で
「未解消の件数と番号」を出し、**起票漏れと未起票の増加で落ちる**。
テスト名やコメントで代用しないこと。テストは本体が経路を叩くので中身が手順書になり、
かつ誰も外さなくても緑のまま通るので、忘れ防止としても弱い。

#### 書く前に自問する

- その文章は、台帳の `sec_detail` / `sec_evidence` / `dynamic_verified` の言い換えになっていないか
- チケット番号に説明を添えていないか。番号を脆弱な実装ファイルの近くに置いていないか
- テストのフィクスチャに、実在の関数名・エンドポイント名・フィールド名を使っていないか
- コミットメッセージに「どの行が該当するか」を書いていないか
  （**コミットメッセージは push したら取り消せない**。本文だけ直しても履歴に残る）
...
更新のコマンドは `docs/OPERATIONS.md` §3 手順 3。
...
|---|---|---|---|
| `api-inventory-drift` | PR / 手動 | 件数、台帳ブランチ名、**未解消のチケット番号** | URI・endpoint 名・台帳の中身・所見 |
...
**実際のリリース手順は `docs/OPERATIONS.md` §4。** 環境の準備は同 §2。 本書が決めるのは「いつ・何を守るか」だけ。

リリースで守るべきルールは次の 4 つ。手順の各ステップは、このどれかを満たすためにある。

| ルール | どこ |
|---|---|
| private 側に同名のブランチを作る | 規則 2-1 / 2-2 |
| 両リポジトリに同名のタグを打つ | 規則 2-3 |
| ゲートが落ちたら原則やり直し。例外はチームリーダの承認 | §3-3 |
| 全レビュー指摘に反応を残してからマージする | §5-2 / §5-4 |
...
- [ ] 台帳の所見を public 側の docstring・コメント・テスト・コミットメッセージに書き写す
- [ ] 未修正の脆弱性のチケット番号を public 側に書く
- [ ] `fixtures.json` を commit する
- [ ] CI に URI や endpoint 名を出させる
- [ ] `reconcile_allow.json` に理由なしで登録する
- [ ] チケット番号に説明を添えて public 側に書く
- [ ] 理由を書かずに `fix_baseline.json` の未起票の上限を上げる
...
| 語 | 意味 |
|---|---|
| **台帳** | `weko3_api_list_full.tsv`(64列) / `weko3_api_list.tsv`(32列)。API の棚卸し結果。列定義は `scripts/schema.py` が唯一の正 |
...
| 2026-09-23 | 未修正のチケット番号 | **番号だけなら public に書いてよい。** 説明は添えず、番号は台帳の `fix_ticket` に集約して一覧としてだけ出す。未修正を private に抱えたまま忘れるほうが危険、という判断（§1） |
| 2026-09-15 | public 側に書いてよい範囲 | ファイルではなく**内容**で判断する。「どう探すか」は public、「どこが該当するか」は private。判定基準は「その記述だけで攻撃が再現できるか」（§1） |
| 2026-09-01 | 本書の置き場所 | `docs/OPERATIONS.md`（2026-09-11 にルールを `docs/RULE.md` へ分離） |
| 2026-09-11 | ルールと手順の分離 | ルールは `docs/RULE.md`、手順は `docs/OPERATIONS.md`。判断の根拠と実行手順を混ぜない（§0） |
