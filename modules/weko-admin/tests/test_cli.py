...
def test_update_attribute_mapping(db, shib_eppn_value=None, shib_role_authority_name_value=None, shib_mail_value=None, shib_user_name_value=None):
    runner = CliRunner()
    result = runner.invoke(
    update_attribute_mapping,
    shib_eppn_value,
    shib_role_authority_name_value,
    shib_mail_value,
    shib_user_name_value
    ) 
   
    assert result.exit_code == 0  
    assert result.output.strip() == "Mapping and update were successful."
    assert AdminSettings.query.filter_by(id=9).one_or_none().name== "attribute_mapping"
