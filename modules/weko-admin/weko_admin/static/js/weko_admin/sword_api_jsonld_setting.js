...
$(document).ready(function () {
  ...
  const urlLoad = '/admin/swordapi/jsonld/add/?url=%2Fadmin%2Fswordapi%2Fjsonld%2F';
  const urlUpdate = '/admin/swordapi/jsonld/add/?url=%2Fadmin%2Fswordapi%2Fjsonld%2F';
...
function saveDataFormat(type) {
  ...
  if (type === "add") {
    url ="/admin/swordapi/jsonld/add/?url=%2Fadmin%2Fswordapi%2Fjsonld%2F";
  } else {
    url ="/admin/swordapi/jsonld/edit/" + current_model_json["id"] + "/?url=%2Fadmin%2Fswordapi%2Fjsonld%2F";
  }
...
window.onload = function() {
  ...
  if ( current_page_type === "add" ) {
    ...
    meta_data_api.forEach(function(api) {
      $('#leftSelect').find('option[value="' + api + '"]').prop("selected", false).appendTo($('#rightSelect'));
    });
    let moveRightDisabled = true;
    if ($('#leftSelect').children().length) {
      moveRightDisabled = false;
