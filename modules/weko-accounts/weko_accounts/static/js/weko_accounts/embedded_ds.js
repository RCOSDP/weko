...
(function() {
	...
	// Else draw embedded WAYF
		...
		if (last_idp == "" && safekind == 2) {
			writeHTML('<img src="https://ds.gakunin.nii.ac.jp/GakuNinDS/images/alert.gif" title="Alert SP!" style="vertical-align:text-bottom; border:0px; width:20px; height:20px;">');
		}
		writeHTML('</label>');
		...
		if (wayf_use_discovery_service == true){
			...
			form_start = '<form id="IdPList" name="IdPList" method="post" target="_parent" onSubmit="return submitForm()" action="' + wayf_authReq_URL + '&amp;time=1583400779'
			+ '">';
		}
		...
		if (wayf_show_remember_checkbox){
			...
			if (wayf_force_remember_for_session){
				...
				writeHTML('<input id="wayf_remember_checkbox" type="checkbox" name="session" value="true" tabindex=8 checked="checked">&nbsp;');
			}
