...
# .tox/c1/bin/pytest --cov=weko_authors tests/test_admin.py::TestAuthorManagementView::test_edit -vv -s --cov-branch --cov-report=term --basetemp=/code/modules/weko-authors/.tox/c1/tmp
    def test_edit(self,client,users,mocker):
        ...
        res = client.get(url)
        mock_render.assert_called_with(
            "weko_authors/admin/author_edit.html",
            identifier_reg='{"ISNI": {"minLength": 0,"maxLength": 30,"reg": "^.*$"},"GRID": {"minLength": 0,"maxLength": 30,"reg": "^.*$"},"Ringgold": {"minLength": 0,"maxLength": 30,"reg": "^.*$"},"kakenhi": {"minLength": 0,"maxLength": 30,"reg": "^.*$"}}',
            render_widgets=False,
            lang_code="en"
        )
...
#        assert_role(res,is_permission)
    ...
    def test_check_status(self,client,users,mocker):
        ...
        res = client.get(url)
        test = {"key":"authors_exported_url","download_link":"xxx"}
        assert json.loads(res.data)==1
        ...
        res = client.get(url)
        test = {"key":"authors_export_status","download_link":"xxx"}
        ...
        res = client.get(url)
        test = {"key":"authors_exported_url","download_link":""}
...
#        assert_role(res,is_permission)
    ...
    def test_export(self,client,users,mocker):
        ...
        res = client.get(url)
        assert json.loads(res.data) == {"code":200,"data":{"taks_id":"test_id"}}
