...
<xsl:stylesheet version="2.0"
                xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:func="http://whatever">
    <xsl:output method="xml" indent="yes"/>

    <xsl:function name="func:change-name">
        <xsl:param name="strN"/>
        <xsl:choose>
                ...
                <xsl:if test="string-length($jc) > 0">
                    <xsl:choose>
                        <xsl:when test="contains($jc,',')">
                            <xsl:variable name="array" select="tokenize($jc,',')"></xsl:variable>
                        ...
                        <xsl:otherwise>
                            <xsl:for-each select="attribute_value">
                                <xsl:choose>
                                    <xsl:when test="contains($jc,'.')">
                                        <xsl:element name="{func:change-name(substring-before($jc,'.'))}">
                                            <xsl:element name="{func:change-name(substring-after($jc,'.'))}">
                                                <xsl:value-of select="."></xsl:value-of>
                                            </xsl:element>
                                        </xsl:element>
                                    </xsl:when>
                                    <xsl:otherwise>
                                        <xsl:element name="{func:change-name($jc)}">
                                            <xsl:value-of select="."></xsl:value-of>
                                        </xsl:element>
                                    </xsl:otherwise>
                                </xsl:choose>
