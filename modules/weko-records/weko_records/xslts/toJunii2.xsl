...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="/">
        <junii2 xmlns="http://irdb.nii.ac.jp/oai"
            ...
            <!-- title:none = title -->
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='title']/doc:element/doc:field">
            ...
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='title']/doc:element[@name='alternative' or @name='transcription']/doc:element/doc:field">
                <alternative>
                    <xsl:value-of select="."/>
                </alternative>
            </xsl:for-each>
            ...
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='contributor']/doc:element[@name='author']/doc:element/doc:field">
                    ...
                    <creator>
                        <xsl:if test="following-sibling::*[2][@name='confidence'] and following-sibling::*[2]/text()&gt;=600">
                            <xsl:choose>
                                ...
                                <xsl:when
                                        test="following-sibling::*[1][@name='authority'] and string-length(following-sibling::*[1]/text())=8">
                                    <xsl:attribute name="id">
                                        <xsl:value-of
                                                select="concat('http://rns.nii.ac.jp/nr/10000', following-sibling::*[1]/text())"/>
                                    </xsl:attribute>
                                </xsl:when>
                                <!-- when using researcher name resolver id as an authority code -->
                                <xsl:when
                                        test="following-sibling::*[1][@name='authority'] and string-length(following-sibling::*[1]/text())=13">
                                    <xsl:attribute name="id">
                                        <xsl:value-of
                                                select="concat('http://rns.nii.ac.jp/nr/', following-sibling::*[1]/text())"/>
                                    </xsl:attribute>
                                </xsl:when>
                            </xsl:choose>
                        </xsl:if>
            ...
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='description']/doc:element[@name='abstract']/doc:element/doc:field">
                <description>
                    <xsl:value-of select="."/>
                </description>
            </xsl:for-each>
            ...
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='publisher']/doc:element/doc:field">
                <publisher>
                    <xsl:value-of select="."/>
                </publisher>
            </xsl:for-each>
            ...
            <!-- type:nii = NIItype -->
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='type']/doc:element[@name='nii']/doc:element/doc:field">
                <NIItype>
                    <xsl:value-of select="."/>
                </NIItype>
            </xsl:for-each>
            <!-- format:none = format -->
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='bundles']/doc:element[@name='bundle']/doc:field[text()='ORIGINAL']">
                <xsl:for-each
                        select="following-sibling::doc:element[@name='bitstreams']/doc:element[@name='bitstream']/doc:field">
            ...
            <xsl:for-each select="doc:metadata/doc:element[@name='dc']/doc:element[@name='identifier']/doc:element">
                <xsl:choose>
                    ...
                    <xsl:when
                            test="@name!='issn' and @name!='uri' and @name!='isbn' and @name!='selfdoi' and @name!='ichushi' and @name!='naid' and @name!='ncid' and @name!='doi' and @name!='pmid' and @name!='scpjid' and @name!='grantid'">
                        <identifier>
                            <xsl:value-of select="./doc:element/doc:field/text()"/>
                        </identifier>
                    </xsl:when>
            ...
            <!-- identifier:uri = URI -->
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='identifier']/doc:element[@name='uri']/doc:element/doc:field">
                <xsl:if test="position() = 1">
                    <URI>
                        <xsl:value-of select="."/>
                    </URI>
            ...
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='identifier']/doc:element[@name='selfdoi']/doc:element/doc:field">
                <xsl:if test="position() = 1">
                    <selfDOI>
            ...
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='identifier']/doc:field[@name='isbn']/doc:element/doc:field">
                <isbn>
                    <xsl:value-of select="."/>
                </isbn>
            </xsl:for-each>
            ...
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='identifier']/doc:element[@name='issn' or @name='eissn']/doc:element/doc:field">
                <issn>
                    <xsl:value-of select="."/>
                </issn>
            </xsl:for-each>
            ...
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='identifier']/doc:element[@name='ncid']/doc:field">
                <NCID>
                    <xsl:value-of select="."/>
                </NCID>
            </xsl:for-each>
            ...
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='source']/doc:element[@name='jtitle']/doc:element/doc:field">
                <xsl:if test="position() = 1">
                    <jtitle>
                        <xsl:value-of select="."/>
                    </jtitle>
            ...
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='source']/doc:element[@name='volume']/doc:element/doc:field">
                <xsl:if test="position() = 1">
                    <volume>
                        <xsl:value-of select="."/>
                    </volume>
            ...
            <!-- source:issue = issue -->
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='source']/doc:element[@name='issue']/doc:element/doc:field">
                <xsl:if test="position() = 1">
                    <issue>
                        <xsl:value-of select="."/>
                    </issue>
            ...
            <!-- source:spage = spage -->
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='source']/doc:element[@name='spage']/doc:element/doc:field">
                <xsl:if test="position() = 1">
                    <spage>
                        <xsl:value-of select="."/>
                    </spage>
            ...
            <!-- source.epage = epage -->
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='source']/doc:element[@name='epage']/doc:element/doc:field">
                <xsl:if test="position() = 1">
                    <epage>
                        <xsl:value-of select="."/>
                    </epage>
            ...
            <!-- date:issued -->
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='date']/doc:element[@name='issued']/doc:element/doc:field">
                <xsl:if test="position() = 1">
                    <dateofissued>
                        <xsl:call-template name="formDate">
            ...
            <xsl:for-each select="doc:metadata/doc:element[@name='dc']/doc:element[@name='source']/doc:element">
                <xsl:choose>
                    ...
                    <xsl:when
                            test="@name!='jtitle' and @name!='volume' and @name!='issue' and @name!='spage' and @name!='epage'">
                        <source>
                            <xsl:value-of select="./doc:element/doc:field/text()"/>
                        </source>
                    </xsl:when>
            ...
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='language']/doc:element[@name='iso']/doc:element/doc:field">
                <xsl:choose>
                    <xsl:when test="string-length(.)=2">
                        <language>
                            <xsl:value-of select="Locale:getISO3Language(Locale:new(.))"/>
                        </language>
                    </xsl:when>
                    <xsl:otherwise>
                        <language>
            ...
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='relation']/doc:element/doc:field">
                <relation>
                    <xsl:value-of select="."/>
                </relation>
            </xsl:for-each>
            ...
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='identifier']/doc:element[@name='pmid']/doc:element/doc:field">
                <xsl:if test="position() = 1">
                    <pmid>
            ...
            <!-- identifier:doi = doi -->
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='identifier']/doc:element[@name='doi']/doc:element/doc:field">
                <xsl:if test="position() = 1">
                    <doi>
            ...
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='identifier']/doc:element[@name='naid']/doc:element/doc:field">
                <xsl:if test="position() = 1">
                    <NAID>
                        <xsl:call-template name="addPrefix">
                            <xsl:with-param name="value" select="."/>
            ...
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='identifier']/doc:element[@name='ichushi']/doc:element/doc:field">
                <xsl:if test="position() = 1">
                    <ichushi>
                        <xsl:call-template name="addPrefix">
                            <xsl:with-param name="value" select="."/>
            ...
            <!-- rights:none = rights -->
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='rights']/doc:elment/doc:field">
                <rights>
                    <xsl:value-of select="."/>
                </rights>
            </xsl:for-each>
            ...
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='description']/doc:element[@name='textversion']/doc:element/doc:field">
                <xsl:if test="position() = 1">
                    <textversion>
                        <xsl:value-of select="."/>
                    </textversion>
            ...
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='identifier']/doc:element[@name='grantid']/doc:element/doc:field">
                <xsl:if test="position() = 1">
                    <grantid>
                        <xsl:value-of select="."/>
                    </grantid>
            ...
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='date']/doc:element[@name='granted']/doc:element/doc:field">
                <xsl:if test="position() = 1">
                    <dateofgranted>
                        <xsl:value-of select="."/>
                    </dateofgranted>
            ...
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='description']/doc:element[@name='degreename']/doc:element/doc:field">
                <xsl:if test="position() = 1">
                    <degreename>
                        <xsl:value-of select="."/>
                    </degreename>
            ...
            <xsl:for-each
                    select="doc:metadata/doc:element[@name='dc']/doc:element[@name='description']/doc:element[@name='grantor']/doc:element/doc:field">
                <xsl:if test="position() = 1">
                    <grantor>
                        <xsl:value-of select="."/>
                    </grantor>
