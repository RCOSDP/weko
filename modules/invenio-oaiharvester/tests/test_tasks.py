...
def test_process_item(app, db, esindex, location, db_itemtype, harvest_setting, db_records, mocker, monkeypatch):
    ...
    mocker.patch("invenio_oaiharvester.tasks.restore")
    mocker.patch("invenio_oaiharvester.tasks.WekoDeposit.update")
    mocker.patch("weko_workflow.utils.IdentifierHandle")
    # jpcoar
    ...
    with pytest.raises(KeyError):
        process_item(_records[0], harvest_setting[0], _counter, None)
    ...
    _counter = {}
    with pytest.raises(KeyError):
        process_item(_records[0], harvest_setting[1], _counter, None)
    ...
    with patch('weko_search_ui.utils.send_item_created_event_to_es', return_value=None):
        with pytest.raises(KeyError):
            process_item(_records[0], harvest_setting[2], _counter, None)
...
def test_run_harvesting(app, db,mocker):
    ...
    responses.add(
        responses.GET,
        "http://export.arxiv.org/oai2/?verb=ListRecords&resumptionToken=test_token",
        body=body2,
        content_type='text/xml'
    )
    body3 = \
