...
$('input[name="access_type"]').change(function () {
    ...
    let accessDate = $(this).parents('.access-type-select').find('input[name="access_date"]');
...
$('#select-all').on('click', function () {
    ...
    let checkboxes = $(this).parent().find('input[type="checkbox"]');
...
$('#submit-btn').on('click', function () {
    ...
    let getUrl = '/admin/items/bulk/update/items_metadata?pids=' + pids;
    $.ajax({
        method: 'GET',
        url: getUrl,
        async: false,
...
// Add field
function addField() {
    let newField = $('.field-row-default').first().clone(true)
