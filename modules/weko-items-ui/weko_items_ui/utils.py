...
def bagify(
    ...
    if not os.path.exists(os.path.join(bag_dir, "data")):
        current_app.logger.eerror(f"Bag directory {bag_dir} does not contain a data directory")
    ...
    try:
        ...
        if unreadable_dirs or unreadable_files:
            ...
            current_app.logger.einfo(_("Creating data directory"))
            ...
            current_app.logger.einfo("Creating bagit.txt")
            txt = """BagIt-Version: 0.97\nTag-File-Character-Encoding: UTF-8\n"""
            with bagit.open_text_file("bagit.txt", "w") as bagit_file:
                bagit_file.write(txt)

            current_app.logger.einfo(_("Creating bag-info.txt"))
            if bag_info is None:
                bag_info = {}
        ...
        current_app.logger.eexception(f"An error occurred creating a bag in {bag_dir}")
        raise
    finally:
        os.chdir(old_dir)
