...
function itemExportCtrl($scope, $rootScope, $http, $location) {
  ...
  /* Used for item export functionality */  //
  ...
  $scope.exportItems = function() {
   if($scope.checkMaxExportItems()) {
     // Export
     // Send all of the data
     alert('Going to export');
     post_data = {
       record_ids: $rootScope.item_export_checkboxes,
       format: $('#export_format_radio_json').is(':checked') ? 'JSON' : 'BIBTEX',
       include_contents: $('#export_file_contents_radio_on').is(':checked') ? true : false
     };
     ...
     $http({
       ...
       // Rest is handled by flask controller
       alert(JSON.stringify(post_data));
     }, function errorCallback(response) {
       // alert(response); // TODO: Do something with the error
     });
   }
   else {
     // Error
     // Pop up modal
     alert('Too many to export');
   }
  }

  $scope.checkMaxExportItems = function() {
  ...
  $scope.checkForRestrictedContent = function(record_id) {
    angular.forEach($scope.vm.invenioSearchResults.hits.hits, function(record) {
      $http({
        ...
        item_index = $scope.items_with_restricted_content.indexOf(record.id);
        if(response.data.has_restricted_content && item_index == -1) {
          $rootScope.items_with_restricted_content.push(record.id);
        }
      }, function errorCallback(response) {
        alert(response); // TODO: Do something with the error
      });
