...
def test_process_researchmap_queue(app ,db, db_records3):
    # item = ItemsMetadata.create(db_records[0][0].object_uuid, id_=rec_uuid)
    db.session.commit()
    # process_researchmap_queue({"item_uuid" : db_records3[0]}  ,MagicMock())
...
def test_build_achievement(app,db_records3):
    recid = PersistentIdentifier.get_by_object(pid_type='recid', object_type='rec', object_uuid=db_records3[0]) 
    record,item = get_item(db_records3[0])
    ...
    recid = PersistentIdentifier.get_by_object(pid_type='recid', object_type='rec', object_uuid=db_records3[1]) 
    record,item = get_item(db_records3[1])
    _ , jrc , _= json_loader(data=item.json ,pid=recid) 
    assert build_achievement(record,item,recid,mapping,jrc, 'published_papers')
    assert build_achievement(record,item,recid,mapping,jrc, 'misc')
    assert build_achievement(record,item,recid,mapping,jrc, 'books_etc')
    ...
    assert build_achievement(record,item,recid,mapping,jrc, 'works')
    assert build_achievement(record,item,recid,mapping,jrc, 'others')
    recid = PersistentIdentifier.get_by_object(pid_type='recid', object_type='rec', object_uuid=db_records3[2]) 
    record,item = get_item(db_records3[2])
    _ , jrc , _= json_loader(data=item.json ,pid=recid) 
    assert build_achievement(record,item,recid,mapping,jrc, 'published_papers')
    assert build_achievement(record,item,recid,mapping,jrc, 'misc')
    assert build_achievement(record,item,recid,mapping,jrc, 'books_etc')
    ...
    # err cases
    recid = PersistentIdentifier.get_by_object(pid_type='recid', object_type='rec', object_uuid=db_records3[3]) 
    record,item = get_item(db_records3[3])
    _ , jrc , _= json_loader(data=item.json ,pid=recid) 
    assert build_achievement(record,item,recid,mapping,jrc, 'published_papers')       
