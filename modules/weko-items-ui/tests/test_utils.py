...
def test_make_stats_file_for_doi_reservation(app, users, db_itemtype, db_records):
    app.config.update(
        EMAIL_DISPLAY_FLG=True,
        WEKO_RECORDS_UI_LICENSE_DICT=[
            {
    ...
    with app.test_request_context():
        assert make_stats_file(item_type_id, [7], list_item_role) is not None

        def filter_f(*args):
            f_magic_mock = MagicMock()

            def f_none():
                return None
            
            f_magic_mock.all = f_none
            f_magic_mock.one_or_none = f_none

            return None
        
        test_data = MagicMock()
        test_data.query = MagicMock()
        test_data.query.filter_by = filter_f

        with patch("weko_items_ui.utils.PersistentIdentifier", MagicMock()):
            assert make_stats_file(item_type_id, [7], list_item_role) is not None
...
# ! TEST FOR DOI RESERVATION
def test_make_stats_file_with_permission_for_doi_reservation(app, users, db_itemtype, db_records):
    item_type_id = 1
    recids = ["7", "2"]
    record0 = WekoRecord.get_record_by_pid(7)
    record1 = WekoRecord.get_record_by_pid(2)
    records_metadata = {"7": record0, "2": record1}
    permissions = dict(
        permission_show_hide=lambda a: True,
        check_created_id=lambda a: True,
        hide_meta_data_for_role=lambda a: True,
    ...
    with app.test_request_context():
        with patch("flask_login.utils._get_user", return_value=users[2]["obj"]):
            assert make_stats_file_with_permission(item_type_id, recids, records_metadata, permissions) is not None
