...
def unpackage_import_file(data_path: str, file_name: str, file_format: str, force_new=False):
    ...
    current_app.logger.debug(f'list_record_9999: {list_record}')
        ...
        if identifierRegistration_key:
            ...
            if item_doi is not None:
                if existed_doi:
                    if item.get("is_change_identifier", False) == True:
                        item["doi"] = item_doi
                    # elif item_doi != doi and item.get("is_change_identifier",False)==False:
            ...
            if item_doi_ra is not None:
                if existed_doi:
                    ...
                    # elif item_doi_ra != doi_ra and item.get("is_change_identifier", False) == False:
                    elif item.get("is_change_identifier", False) == False:
                        item["doi_ra"] = doi_ra
                        # item["doi_ra"] = item_doi_ra
                        fixed_doi_ra = True
