    ...
    with patch('weko_search_ui.query.search_permission.can', return_value=True):
        with patch("flask_login.utils._get_user", return_value=users[3]['obj']):
            ...
            with patch("weko_search_ui.query.check_permission_user",return_value=(users[3]["id"],True)):
                ...
                # not exist index_id
                res = get_permission_filter()
                assert res == ([Bool(must=[Terms(path=["33","44"])], should=[Bool(must=[Terms(publish_status=['0', '1']), Match(weko_creator_id=5)]), Bool(must=[Terms(publish_status=['0', '1']), Match(weko_shared_id=5)]), Bool(must=[Terms(publish_status=['0', '1'])])]), Bool(must=[Match(relation_version_is_last='true')])], ["33", "33/44"])
        # not admin user
