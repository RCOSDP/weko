        ...
        with patch("flask_login.utils._get_user", return_value=users[3]['obj']):
            ...
            with patch("weko_search_ui.query.check_permission_user",return_value=(users[3]["id"],True)), \
                    patch("weko_index_tree.api.Indexes.get_browsing_tree_paths",
                          return_value=["33", "33/44"]):
                # get_browsing_tree_paths を差し替えないと閲覧可能なインデックスが
                # ['1'] だけになり、index_id ('33') が is_perm_indexes に入らず
                # 条件がまるごと飛ぶ。他の get_permission_filter 系のテストと
                # 同じように差し替える。
                # exist index_id, search_type = Full_TEXT
                with i18n_app.test_request_context("/test?search_type=0"):
                    # index_id in is_perm_indexes
