...
from weko_search_ui.utils import (
    ...
    handle_check_custom_sort_order,
...
def test_parse_to_json_form(i18n_app, record_with_metadata):
    ...
    data3 = [
        ("metadata.path[0]", "1616224532673"),
        ("metadata.path[1]", "1715825846862"),
        ("custom_sort_order[0]", "1"),
        ("custom_sort_order[1]", "2"),
        ("custom_sort_order[2]", "3")
    ]
    ret = parse_to_json_form(data3)
    assert ret["index_sort"] == {'1616224532673': '1','1715825846862': '2'}
...
def test_register_item_custom_sort_order(i18n_app, es_item_file_pipeline, es_records):
    ...
    with patch("weko_index_tree.api.Indexes.set_item_sort_custom", return_value=""):
        assert not register_item_custom_sort_order(item)
        item["index_sort"] = {"12345":""}
        assert not register_item_custom_sort_order(item)
...
# .tox/c1/bin/pytest --cov=weko_search_ui tests/test_utils.py::test_handle_check_custom_sort_order -vv -s --cov-branch --cov-report=term --basetemp=/code/modules/weko-search-ui/.tox/c1/tmp
def test_handle_check_custom_sort_order(i18n_app, record_with_metadata):
    record = record_with_metadata[0]
    assert not handle_check_custom_sort_order([record])
    record["custom_sort_order"] = ["1","2"]
    assert not handle_check_custom_sort_order([record])
    record["custom_sort_order"] = "-1"
    assert not handle_check_custom_sort_order([record])
    record["custom_sort_order"] = ["三"]
    assert not handle_check_custom_sort_order([record])
    record["custom_sort_order"] = ["0"]
    assert not handle_check_custom_sort_order([record])
