...
# @pytest.mark.parametrize('order_if', [1,2,3,4,5,6,7,8,9])
#def check_tsv_import_items(file, is_change_identifier: bool, is_gakuninrdm=False):
# .tox/c1/bin/pytest --cov=weko_search_ui tests/test_utils.py::test_check_tsv_import_items3 -vv -s --cov-branch --cov-report=term --basetemp=/code/modules/weko-search-ui/.tox/c1/tmp
def test_check_tsv_import_items3(app,test_importdata,mocker,db):
    app.config['WEKO_SEARCH_UI_IMPORT_TMP_PREFIX'] = 'importtest'
    filepath = os.path.join(os.path.dirname(os.path.realpath(__file__)),"data", "item_map.json")
    with open(filepath,encoding="utf-8") as f:
        item_map = json.load(f)
    ...
    with app.test_request_context():
        with set_locale('en'):
            ...
            mocker.patch("weko_search_ui.utils.handle_check_doi_ra")
            mocker.patch("weko_search_ui.utils.handle_check_doi")
            check_authors_prefix = mocker.patch("weko_search_ui.utils.handle_check_authors_prefix")
            check_authors_affiliation = mocker.patch("weko_search_ui.utils.handle_check_authors_affiliation")
            for file in test_importdata:
                check_tsv_import_items(file,False,False)==''
                check_authors_prefix.assert_called()
                check_authors_affiliation.assert_called()
...
# .tox/c1/bin/pytest --cov=weko_search_ui tests/test_utils.py::test_handle_check_authors_prefix -vv -s --cov-branch --cov-report=term --basetemp=/code/modules/weko-search-ui/.tox/c1/tmp
def test_handle_check_authors_prefix(db_author):
    list_record = []
    handle_check_authors_prefix(list_record)
    assert list_record == []

    list_record = json_data("data/list_records/list_records_authors.json")
    handle_check_authors_prefix(list_record)
    item = list_record[0]
    assert "errors" not in item or not item.get("errors")

    list_record = json_data("data/list_records/list_records_authors.json")
    with patch("weko_search_ui.utils.AuthorsPrefixSettings.query") as mock_query:
        mock_query.all.return_value = []
        handle_check_authors_prefix(list_record)
        item = list_record[0]
        errors= [
            '"ORCID" is not one of [] in item_1617349709064',
            '"CiNii" is not one of [] in item_1617349709064',
            '"KAKEN2" is not one of [] in item_1617349709064',
            '"ORCID" is not one of [] in item_1617610673286',
            '"WEKO" is not one of [] in item_1617186419668',
            '"ORCID" is not one of [] in item_1617186419668',
            '"CiNii" is not one of [] in item_1617186419668',
            '"KAKEN2" is not one of [] in item_1617186419668',
            '"ORCID" is not one of [] in item_1617186419668',
            '"CiNii" is not one of [] in item_1617186419668',
            '"KAKEN2" is not one of [] in item_1617186419668',
            '"ORCID" is not one of [] in item_1617186419668',
        ...
        assert sorted(item.get("errors")) == sorted(errors)


    list_record = json_data("data/list_records/list_records_authors_error.json")
    allowed_scheme = "[\'WEKO\', \'ORCID\', \'CiNii\', \'KAKEN2\', \'ROR\']"
    print(f"list_record2475: {list_record}")
    handle_check_authors_prefix(list_record)
    item = list_record[0]
    errors= [
        f'"WEKOX" is not one of {allowed_scheme} in item_1617186419668',
        f'"KAKEN2X" is not one of {allowed_scheme} in item_1617186419668'
    ]
    assert sorted(item.get("errors")) == sorted(errors)


# def handle_check_authors_affiliation(list_record):
# .tox/c1/bin/pytest --cov=weko_search_ui tests/test_utils.py::test_handle_check_authors_affiliation -vv -s --cov-branch --cov-report=term --basetemp=/code/modules/weko-search-ui/.tox/c1/tmp
def test_handle_check_authors_affiliation(db_author):
    list_record = []
    handle_check_authors_affiliation(list_record)
    assert "errors" not in list_record or not list_record.get("errors")
    ...
    list_record = json_data("data/list_records/list_records_authors.json")
    with patch("weko_search_ui.utils.AuthorsAffiliationSettings.query") as mock_query:
        mock_query.all.return_value = []
        handle_check_authors_affiliation(list_record)
        item = list_record[0]
        errors= ['"ISNI" is not one of [] in item_1617186419668']
        assert sorted(item.get("errors")) == sorted(errors)

    list_record = json_data("data/list_records/list_records_authors_error.json")
    allowed_scheme = "[\'ISNI\', \'GRID\', \'Ringgold\', \'kakenhi\']"
    handle_check_authors_affiliation(list_record)
    item = list_record[0]
    errors= [f'"ISNIX" is not one of {allowed_scheme} in item_1617186419668']
    assert sorted(item.get("errors")) == sorted(errors)
