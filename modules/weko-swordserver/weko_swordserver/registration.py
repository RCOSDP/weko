...
from weko_search_ui.utils import (
    get_file_list_of_zip,
    get_unpacked_zip_path,
    check_rocrate_bagit_required_files,
    check_swordbagit_required_files,
    check_manifest_sha_256,
    check_rocrate_bagit_import_items,
...
def check_import_items(file, is_change_identifier: bool = False):
    ...
    file_format, file_list = check_import_file_format(file, packaging)
    ...
    if file_format != 'OTHERS':
        ...
        if file_format == 'ROCRATE':
            file_exist_result = check_rocrate_bagit_required_files(file_list)
        elif file_format == 'SWORD':
            file_exist_result = check_swordbagit_required_files(file_list)
        else:
            raise ValueError('Result of file_format is incorrect')

        if not all(file_exist_result):
            raise FileNotFoundError('Missing Some File')

        # Unpack Zip File
        data_path = get_unpacked_zip_path(file)

        if data_path == None:
            raise FileNotFoundError('Can not get data path')

        # Check manifest-sha-256.txt
        manifest_path = os.path.join(data_path, 'manifest-sha-256.txt')
        if not check_manifest_sha_256(manifest_path):
            raise ValueError

        # Check tagmanifest-sha-256.txt
        tagmanifest_path = os.path.join(data_path, 'tagmanifest-sha-256.txt')
        if not check_manifest_sha_256(tagmanifest_path):
            raise ValueError

        # Check if Workflow
        workflow_id = int(data_format.get("XML", {}).get("workflow", "-1"))
        workflow = WorkFlow.query.get(workflow_id)  #こいつはidがworkflow_idと一致するWorkFlowのレコード
        if not workflow or workflow.is_deleted:
            ...
            # 直接登録の場合のitem_type_idは？？
            return 'Direct'
        item_type_id = workflow.itemtype_id

        # Get Mapping Result
        check_result = check_rocrate_bagit_import_items(file, item_type_id, is_gakuninrdm=False)  #ここはweko_swordserver/utils.pyにできる予定のものに合わせる
        return check_result, 'Workflow'

    else:
        return check_others_import_items(file, is_change_identifier)
...
def check_digest():
    """Validate Digest in request headers.
    ...
    body_hash = hashlib.sha256(request.data).digest()
    body_hash_base64 = base64.b64encode(body_hash).decode()

    if not digest_header:
        raise WekoSwordserverException("Digest Mismatched.", ErrorType.DigestMismatch)  #一旦同じエラーにする

    if not digest_header.startwith('SHA-256='):
        raise WekoSwordserverException("Digest Mismatched.", ErrorType.DigestMismatch)

    digest_value = digest_header.split('SHA-256=')

    if body_hash_base64 != digest_value:
        raise WekoSwordserverException("Digest Mismatched.", ErrorType.DigestMismatch)
...
def check_import_file_format(file, packaging):
    ...
    if packaging == 'SWORDBagIt':
        ...
        raise WekoSwordserverException("No Package Included.", ErrorType.BadRequest)
