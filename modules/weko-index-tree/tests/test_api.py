...
def test_indexes_move(app, db, users):
    with app.test_client() as client:
        login_user_via_session(client=client, email=users[3]['email'])
        Indexes.move(3, **data)
        Indexes.create(4, {
...
# .tox/c1/bin/pytest --cov=weko_index_tree tests/test_api.py::test_indexes_get_index_with_role -v -s -vv --cov-branch --cov-report=term --cov-config=tox.ini --basetemp=/code/modules/weko-index-tree/.tox/c1/tmp
