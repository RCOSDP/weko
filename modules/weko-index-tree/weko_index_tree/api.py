...
class Indexes(object):
    ...
    def recs_tree_query(cls, pid=0, ):
        ...
        if lang == 'ja':
            recursive_t = db.session.query(
                ...
                Index.id.label("cid"),
                func.cast(Index.id, db.Text).label("path"),
                func.coalesce(Index.index_name, Index.index_name_english).label(
                    "name"),
                Index.position,
                Index.public_state,
                Index.public_date,
        ...
        try:
            ...
            for d in sort_json:
                for h in res.get("hits").get("hits"):
                    if int(h.get('_source').get('control_number')) == \
                            int(d.get("id")):
                        body = {
                            'doc': {
    ...
    def get_item_sort(cls, index_id):
        ...
        """
        item_custom_sort = db.session.query(Index.item_custom_sort).filter(
