...
function preparePostData(tmp_save, actionButton) {
    ...
    if (tmp_save == 1 || idf_grant_method == 0) {
        ...
        if (identifier_grant == "4") {
            let isSuffixFormatNDL = isDOISuffixFormat(identifier_grant_ndl_jalc_doi_link, identifier_grant_ndl_jalc_doi_suffix, actionButton)
...
function validateLengDoi(arrayDoi, actionButton) {
    let msg = '';
    let result = true;
    for (let index = 0; index < arrayDoi.length; ++index) {
