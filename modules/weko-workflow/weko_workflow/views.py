...
from weko_workflow.schema.marshmallow import (
    ...
    SaveActivitySchema,
    CheckApprovalSchema,
    ResponseUnlockSchema,
    GetItemApplicationSchema,
)
...
from weko_deposit.pidstore import get_record_identifier, get_record_without_version
...
from .api import Action, Flow, GetCommunity, WorkActivity, WorkActivityHistory, WorkFlow
from .config import (
    IDENTIFIER_GRANT_LIST,
    IDENTIFIER_GRANT_SELECT_DICT,
    IDENTIFIER_GRANT_SUFFIX_METHOD,
    WEKO_WORKFLOW_TODO_TAB,
...
def index():
    ...
    page, render_widgets = get_design_layout(
        request.args.get("c") or current_app.config["WEKO_THEME_DEFAULT_COMMUNITY"]
    )
    ...
    send_mail_user_group = current_app.config.get("WEKO_WORKFLOW_SEND_MAIL_USER_GROUP")
    # WEKO_WORKFLOW_ENABLE_SHOW_ACTIVITY = False
    enable_show_activity = current_app.config["WEKO_WORKFLOW_ENABLE_SHOW_ACTIVITY"]
    ...
    if settings:
        ...
        activity_display_flg = current_app.config.get(
            "WEKO_WORKFLOW_APPROVER_EMAIL_COLUMN_VISIBLE"
        )
        ...
        for step in steps:
            if step["ActionName"] == "Approval":
                ...
                if approver is not None:
                    ...
                    if (
                        hasattr(approver_profile, "_displayname")
                        and approver_profile._displayname
                    ):
    ...
    return render_template(
        ...
        activitylog_roles=current_app.config.get(
            "WEKO_WORKFLOW_ACTIVITYLOG_ROLE_ENABLE"
        ),
        **ctx
    )
...
def iframe_success():
    ...
    if not session.keys() >= {
        ...
        return render_template(
            "weko_theme/error.html", error="can not get data required for rendering"
        )
    ...
    action_comment = (
        activity_action.action_comment
        if activity_action and activity_action.action_comment
        else ""
    )
    ...
    record_detail_alt = get_main_record_detail(activity.activity_id, activity)
    ctx.update(
        dict(
    ...
    return render_template(
        "weko_workflow/item_login_success.html",
        page=page,
        render_widgets=render_widgets,
        activity=activity,
        item=item,
...
def init_activity(json_data=None, community=None):
        ...
        res = ResponseMessageSchema().load(
            {
                "code": 1,
                "msg": "success",
                "data": {"is_download": True, "redirect": url},
            }
        )
    ...
    try:
        ...
        if rtn is None:
            res = ResponseMessageSchema().load(
                {"code": -1, "msg": "can not make activity_id"}
            )
            return jsonify(res.data), 500
        url = url_for("weko_workflow.display_activity", activity_id=rtn.activity_id)
        ...
        db.session.rollback()
        res = ResponseMessageSchema().load(
            {"code": -1, "msg": "sqlalchemy error: {}".format(ex)}
        )
        return jsonify(res.data), 500
        ...
        db.session.rollback()
        res = ResponseMessageSchema().load(
            {"code": -1, "msg": "Unexpected error: {}".format(ex)}
        )
        return jsonify(res.data), 500
...
def _generate_download_url(record_id: str, file_name: str) -> str:
    ...
    url: str = url_for(
        "invenio_records_ui.recid_files", pid_value=record_id, filename=file_name
    )
...
@workflow_blueprint.route(
    "/activity/guest-user/recid/<string:record_id>", methods=["GET"]
)
...
def render_guest_workflow(file_name=""):
    ...
    if not is_valid:
        return render_template("weko_theme/error.html", error=_("Token is invalid"))

    error_msg = validate_guest_activity_expired(activity_id)
    if error_msg:
        return render_template("weko_theme/error.html", error=error_msg)
    ...
    record_detail_alt = get_main_record_detail(
        activity_id, guest_activity.get("activity")
    )
    ...
    if not guest_activity.get("steps", []):
        current_app.logger.error(
            "can not get workflow_action_history:{}".format(activity_id)
        )
        abort(404)

    result = render_template(
        "weko_workflow/activity_detail.html", form=form, **guest_activity
    )
...
@workflow_blueprint.route(
    "/activity/detail/<string:activity_id>", methods=["GET", "POST"]
)
@login_required_customize
def display_activity(activity_id="0", community_id=None):
    ...
    for_delete = (
        activity_detail.flow_define.flow_type == WEKO_WORKFLOW_DELETION_FLOW_TYPE
    )
    ...
    if action_endpoint in ["item_login", "item_login_application", "file_upload"]:
        if not activity.get_activity_by_id(activity_id):
            pass
        if (
            activity.get_activity_by_id(activity_id).action_status
        ...
        sessionstore = redis_connection.connection(
            db=current_app.config["ACCOUNTS_SESSION_REDIS_DB_NO"], kv=True
        )

        if not (json_schema and schema_form):
            current_app.logger.error(
                "display_activity: can not get json_schema,schema_form"
            )
            return render_template(
                "weko_theme/error.html", error="can not get data required for rendering"
            )
    ...
    if (
        activity_detail
        and activity_detail.item_id
        and activity_detail.activity_status != ActivityStatusPolicy.ACTIVITY_CANCEL
    ):
        try:
            if not for_delete:
    ...
    res_check = check_authority_action(
        str(activity_id),
        int(action_id),
    ...
    if (
        "item_login" == action_endpoint
        or "item_login_application" == action_endpoint
        or "file_upload" == action_endpoint
    ):
        cur_locked_val = str(get_cache_data(user_lock_key)) or str()
        if not cur_locked_val:
    ...
    try:
        if activity_detail.activity_status != ActivityStatusPolicy.ACTIVITY_CANCEL:
            ...
            if not record_detail_alt:
                current_app.logger.error(
                    "display_activity: bad value for record_detail_alt"
                )
                return render_template(
                    "weko_theme/error.html",
    ...
    ctx.update(
        dict(files_thumbnail=files_thumbnail, files=files, record=approval_record)
    )
    _id = None
    if recid:
    ...
    if (
        getattr(activity_detail, "extra_info", "")
        and activity_detail.extra_info
        and action_endpoint == "end_action"
    ):
        applied_record_id = activity_detail.extra_info.get("record_id", -1)
        record = WekoRecord.get_record_by_pid(applied_record_id)
        if record:
            url_to_item_to_apply_for = urljoin(
                request.url_root,
                url_for("invenio_records_ui.recid", pid_value=record.pid.pid_value),
            )
    ...
    restricted_access_settings = AdminSettings.get(
        name="restricted_access", dict_to_object=False
    )
    if restricted_access_settings:
        if action_endpoint == "approval" and restricted_access_settings.get(
            "preview_workflow_approval_enable", False
        ):
            approval_preview = workflow_detail.open_restricted
        enable_request_maillist = restricted_access_settings.get(
            "display_request_form", False
        )
        item_application_settings = restricted_access_settings.get(
            "item_application", {}
        )
        is_no_content_item_application = item_application_settings.get(
            "item_application_enable", False
        ) and workflow_detail.itemtype_id in item_application_settings.get(
            "application_item_types", []
        )

    last_result: CRISLinkageResult = CRISLinkageResult().get_last(
        _id, CRIS_Institutions.RM
    )
    ...
    if last_result:
        last_linkage_result = (
            _("Successful")
            if last_result.succeed == True
            else _("Failed") if last_result.succeed == False else _("Running")
        )
        last_linkage_result = (
            last_linkage_result + " (" + last_result.updated.strftime("%Y-%m-%d") + ") "
        )

    return render_template(
        "weko_workflow/activity_detail.html",
        action_endpoint_key=current_app.config.get("WEKO_ITEMS_UI_ACTION_ENDPOINT_KEY"),
        action_id=action_id,
        ...
        enable_contributor=current_app.config["WEKO_WORKFLOW_ENABLE_CONTRIBUTOR"],
        enable_feedback_maillist=current_app.config[
            "WEKO_WORKFLOW_ENABLE_FEEDBACK_MAIL"
        ],
        enable_multi_contributors=current_app.config["WEKO_ITEMS_UI_PROXY_POSTING"],
...
def check_authority_action(
    activity_id="0", action_id=0, contain_login_item_application=False, action_order=0
):
    """Check authority."""
    ...
    if (
        current_app.config["WEKO_WORKFLOW_ENABLE_CONTRIBUTOR"]
        and action_id != _Action.query.filter_by(action_endpoint="approval").one().id
    ):
        ...
        if not im and activity.temp_data:
            ...
            activity_shared_user_ids = (
                activity.shared_user_ids if activity.shared_user_ids else []
            )
            activity_user_ids = _get_shared_user_ids_from_list(activity_shared_user_ids)
            ...
            if temp_data is not None:
                ...
                temp_user_ids = _get_shared_user_ids_from_list(temp_shared_user_ids)
    ...
    is_action_role_set, is_allow_action_role, is_deny_action_role = (
        validate_action_role_user(activity_id, action_id, action_order)
    )
    ...
    if contain_login_item_application:
        ...
        if (
            activity_action_obj
            and activity_action_obj.action_handler
...
@workflow_blueprint.route(
    "/activity/action/<string:activity_id>/<int:action_id>", methods=["POST"]
)
@login_required_customize
@check_authority
def next_action(activity_id="0", action_id=0, json_data=None):
    ...
    if activity_detail is None:
        ...
        res = ResponseMessageSchema().load(
            {"code": -1, "msg": "can not get activity detail"}
        )
        return jsonify(res.data), 500
    for_delete = (
        activity_detail.flow_define.flow_type == WEKO_WORKFLOW_DELETION_FLOW_TYPE
    ...
    try:
        ...
        if schema is None:
            ...
            res = ResponseMessageSchema().load(
                {"code": -2, "msg": "can not get schema by action_id"}
            )
            return jsonify(res.data), 500
    ...
    if activity_detail and activity_detail.item_id:
        ...
        try:
            ...
            res = ResponseMessageSchema().load(
                {"code": -1, "msg": "can not get PersistentIdentifier"}
            )
            return jsonify(res.data), 500
    ...
    if pid_without_ver is None:
        ...
        res = ResponseMessageSchema().load(
            {"code": -1, "msg": "can not get pid_without_ver"}
        )
        return jsonify(res.data), 500

    current_app.logger.debug(
        "action_endpoint: {0}, current_pid: {1}, item_id: {2}".format(
    ...
    if (
        action_endpoint in ["item_login", "item_login_application"]
    ...
    if current_flow_action is None:
        ...
        res = ResponseMessageSchema().load(
            {"code": -1, "msg": "can not get curretn_flow_action"}
        )
        return jsonify(res.data), 500
    ...
    if not isinstance(next_flow_action, list) or len(next_flow_action) <= 0:
        ...
        res = ResponseMessageSchema().load(
            {"code": -2, "msg": "can not get next_flow_action"}
        )
        return jsonify(res.data), 500
    next_action_endpoint = next_flow_action[0].action.action_endpoint
    next_action_id = next_flow_action[0].action_id
    next_action_order = next_flow_action[0].action_order if action_order else None
    action_mails_setting = {
        ...
        "next": (
            next_flow_action[0].send_mail_setting
    ...
    # Start to send mail
    if (
        next_action_endpoint in ["approval", "end_action"]
        and post_json.get("temporary_save") == 0
    ):
        next_action_detail = work_activity.get_activity_action_comment(
            activity_id, next_action_id, next_action_order
        )

        if next_action_detail is None:
            current_app.logger.error("next_action: can not get next_action_detail")
            res = ResponseMessageSchema().load(
                {"code": -2, "msg": "can not get next_action_detail"}
            )
            return jsonify(res.data), 500

        enable_restricted = current_app.config.get(
        ...
        if is_last_step and enable_restricted:
            ...
            permissions: List[FilePermission] = FilePermission.find_by_activity(
                activity_id
            )
            ...
            if (
                extra_info
                and extra_info.get("file_name")
                and not re.fullmatch(
                    r"recid/\d+(?:\.\d+)?", extra_info.get("file_name")
                )
            ):
                if permissions and len(permissions) == 1:
                    ...
                    url_and_expired_date = (
                        grant_access_rights_to_all_open_restricted_files(
                            activity_id, permissions[0], activity_detail
                        )
                    )
                    ...
                    url_and_expired_date = (
                        grant_access_rights_to_all_open_restricted_files(
                            activity_id, guest_activity[0], activity_detail
                        )
                    )
        ...
        restricted_access_setting = (
            AdminSettings.get("restricted_access", dict_to_object=False) or {}
        )
        ...
        if next_action_handler == -1:
            ...
            if (
                current_flow_action
                and current_flow_action.action_roles
                and current_flow_action.action_roles[0].action_request_mail
            ):
                is_request_enabled = restricted_access_setting.get(
                    "display_request_form", False
                )
                # リクエスト機能がAdmin画面で無効化されている場合、メールは送信しない。
                if is_request_enabled:
                    next_action_handler = (
                        work_activity.get_user_ids_of_request_mails_by_activity_id(
                            activity_id
                        )
                    )
                else:
            ...
            if (
                current_flow_action
                and current_flow_action.action_roles
                and current_flow_action.action_roles[0].action_user
            ):
                next_action_handler = current_flow_action.action_roles[0].action_user
        enable_mail_templates = restricted_access_setting.get(
        ...
        if enable_restricted and enable_mail_templates:
            ...
            if type(next_action_handler) == list:
                for handler in next_action_handler:
                    roles, users = work_activity.get_activity_action_role(
                        activity_id, next_action_id, current_flow_action.action_order
                    )
    ...
    if (
        current_app.config.get("WEKO_WORKFLOW_ENABLE_AUTO_SEND_EMAIL")
        and current_user.is_authenticated
        and (
            not activity_detail.extra_info
            or not activity_detail.extra_info.get("guest_mail")
        )
    ):
        process_send_notification_mail(
            activity_detail, action_endpoint, next_action_endpoint, action_mails_setting
        )

    if post_json.get("temporary_save") == 1 and action_endpoint not in [
        "identifier_grant",
        "item_link",
    ]:
    ...
    if action_endpoint == "approval" and item_id and not for_delete:
        last_idt_setting = work_activity.get_action_identifier_grant(
            activity_id=activity_id, action_id=get_actionid("identifier_grant")
        )
        if (
            not post_json.get("temporary_save")
            and last_idt_setting
            ...
            current_app.logger.debug("last_idt_setting: {0}".format(last_idt_setting))
            saving_doi_pidstore(
                item_id,
        ...
        elif last_idt_setting and last_idt_setting.get("action_identifier_select"):
            ...
            if last_idt_setting.get("action_identifier_select") == -2:
                ...
                current_app.logger.debug("delete_pidstore_doi: {0}".format(del_doi))
        ...
        if action_feedbackmail or activity_request_mail or activity_item_application:
            ...
            if not recid:
                if ".0" in current_pid.pid_value:
                    ...
                    last_ver = (
                        PIDVersioning(parent=pv.parent, child=pid_without_ver)
                    ...
                    if last_ver is None:
                        res = ResponseMessageSchema().load(
                            {"code": -1, "msg": "can not get last_ver"}
                        )
                        return jsonify(res.data), 500
                    item_ids.append(last_ver.object_uuid)
                else:
                    draft_pid = PersistentIdentifier.get(
                        "recid", "{}.0".format(pid_without_ver.pid_value)
                    )
            ...
            restricted_access_settings = AdminSettings.get(
                "restricted_access", dict_to_object=False
            )
            if restricted_access_settings:
                enable_request_maillist = restricted_access_settings.get(
                    "display_request_form", False
                ...
                enable_item_application = item_application_settings.get(
                    "item_application_enable", False
                )
                application_item_types = item_application_settings.get(
                    "application_item_types", []
                )
                can_register_item_application = enable_item_application and (
                    activity_detail.workflow.itemtype_id in application_item_types
                )

            if (
                activity_request_mail
                and activity_request_mail.request_maillist
                and enable_request_maillist
            ):
            ...
            if (
                activity_item_application
                and activity_item_application.item_application
                and can_register_item_application
            ):
    ...
    if "identifier_grant" == action_endpoint and identifier_select is not None:
        ...
        work_activity.create_or_update_action_identifier(
            activity_id=activity_id, action_id=action_id, identifier=identifier_grant
        )
        if post_json.get("temporary_save") == 1:
            res = ResponseMessageSchema().load({"code": 0, "msg": _("success")})
            return jsonify(res.data), 200
        ...
        if identifier_select == IDENTIFIER_GRANT_SELECT_DICT["NotGrant"]:
            if item_id != pid_without_ver.object_uuid:
                ...
                if not _old_v:
                    ...
                    _new_idt.update_idt_registration_metadata(_old_v, _old_t)
            else:
            ...
            if isinstance(error_list, str):
                ...
                return previous_action(
                    activity_id=activity_id, action_id=action_id, req=-1
                )
            ...
            if not recid:
                record_without_version = pid_without_ver.object_uuid
            saving_doi_pidstore(
                item_id, record_without_version, post_json, int(identifier_select), True
            )
    elif "identifier_grant" == action_endpoint and not post_json.get("temporary_save"):
        _value, _type = IdentifierHandle(item_id).get_idt_registration_data()
        if _value and _type:
            error_list = check_doi_validation_not_pass(
                item_id,
                activity_id,
                IDENTIFIER_GRANT_SELECT_DICT[_type[0]],
    ...
    if next_action_endpoint == "end_action" and for_delete and del_reject_flg == False:
        ...
        UserActivityLogger.info(
            operation="ITEM_DELETE", target_key=current_pid.pid_value
        )
    ...
    if for_delete and del_reject_flg:
        ...
        skip_acts = [
            act
            for act in flow_detail.flow_actions
        ...
        activity.update(action_status=ActionStatusPolicy.ACTION_THROWN_OUT)
    ...
    flag = work_activity.upt_activity_action_status(
        ...
        action_status=(
            ...
            if del_reject_flg and for_delete
    ...
    if next_action_endpoint == "end_action":
        if not for_delete:
            ...
            new_item_id = handle_finish_workflow(deposit, current_pid, recid)
            ...
            if temp_data:
                if (
                    orjson.loads(temp_data)
                    .get("cris_linkage", {})
                    .get("researchmap", False)
                ):
            ...
            try:
                if "." not in current_pid.pid_value and has_request_context():
                    user_id = (
                        activity_detail.activity_login_user
                        if activity and activity_detail.activity_login_user
                        else -1
                    )
                    item_created.send(
...
@workflow_blueprint.route(
    "/activity/action/<string:activity_id>/<int:action_id>" "/rejectOrReturn/<int:req>",
    methods=["POST"],
)
@login_required
@check_authority
def previous_action(activity_id="0", action_id=0, req=0):
    ...
    activity = dict(
        ...
        action_status=(
            ActionStatusPolicy.ACTION_THROWN_OUT
            if 0 == req
            else ActionStatusPolicy.ACTION_RETRY
        ),
    ...
    if current_flow_action is None:
        ...
        res = ResponseMessageSchema().load(
            {"code": -1, "msg": "can not get flow action detail"}
        )
        return jsonify(res.data), 500
    action_mails_setting = {
        "previous": (
    ...
    if pre_action and len(pre_action) > 0:
        ...
        previous_action_order = pre_action[0].action_order if action_order else None
        if req == 0:
            flag = work_activity.upt_activity_action_status(
                activity_id=activity_id,
                action_id=action_id,
...
def get_journal(method, value):
    ...
    if result["romeoapi"] and int(result["romeoapi"]["header"]["numhits"]) > 1:
        if isinstance(result["romeoapi"]["journals"]["journal"], list):
            result["romeoapi"]["journals"]["journal"] = result["romeoapi"]["journals"][
                "journal"
            ][0]
        if isinstance(result["romeoapi"]["publishers"]["publisher"], list):
            result["romeoapi"]["publishers"]["publisher"] = result["romeoapi"][
                "publishers"
            ]["publisher"][0]
...
@workflow_blueprint.route(
    "/activity/action/<string:activity_id>/<int:action_id>" "/cancel", methods=["POST"]
)
@login_required_customize
@check_authority
def cancel_action(activity_id="0", action_id=0):
    ...
    if cancel_item_id is None:
        pid_value = (
            post_json.get("pid_value")
            if post_json.get("pid_value")
            else get_pid_value_by_activity_detail(activity_detail)
        )
        if pid_value:
            try:
                ...
                res = ResponseMessageSchema().load(
                    {"code": -1, "msg": "can not get PersistIdentifier"}
                )
                return jsonify(res.data), 500
    ...
    if cancel_record is None or not (
        for_delete and not cancel_record.pid.pid_value.endswith(".0")
    ):
        try:
            with db.session.begin_nested():
                if cancel_record:
                    ...
                    if cancel_deposit.files and cancel_deposit.files.bucket:
                        remove_file_cancel_action(cancel_deposit.files.bucket.id)
                    ...
                    try:
                        ...
                        current_app.logger.error(
                            "cancel_action: can not get PersistentIdentifier"
                        )
                        res = ResponseMessageSchema().load(
                            {"code": -1, "msg": "can not get PersistentIdentifier"}
                        )
                ...
                pids = PersistentIdentifier.query.filter_by(object_uuid=cancel_item_id)
                for p in pids:
    ...
    if not rtn:
        ...
        res = ResponseMessageSchema().load(
            {"code": -1, "msg": _("Error! Cannot process quit activity!")}
        )
        return jsonify(res.data), 500
    ...
    if cur_locked_val and cur_locked_val == activity_id:
        update_cache_data(cache_key, cur_locked_val, 1)
...
@workflow_blueprint.route(
    "/activity/detail/<string:activity_id>/<int:action_id>" "/withdraw",
    methods=["POST"],
)
@login_required_customize
@check_authority
def withdraw_confirm(activity_id="0", action_id=0):
    ...
    try:
        ...
        if password == "DELETE":
            ...
            res = ResponseMessageSchema().load(
                {"code": -1, "msg": _("Invalid password")}
            )
            return jsonify(res.data), 500
    except ValueError:
        current_app.logger.error(
            "withdraw_confirm: Unexpected error: {}".format(sys.exc_info())
        )
    res = ResponseMessageSchema().load({"code": -1, "msg": _("Error!")})
    return jsonify(res.data), 500
...
@workflow_blueprint.route(
    "/save_feedback_maillist/<string:activity_id>/<int:action_id>", methods=["POST"]
)
@login_required
@check_authority
...
@workflow_blueprint.route(
    "/save_request_maillist/<string:activity_id>/<int:action_id>", methods=["POST"]
)
@login_required
@check_authority
...
@workflow_blueprint.route(
    "/save_item_application/<string:activity_id>/<int:action_id>", methods=["POST"]
)
@login_required
@check_authority
def save_item_application(activity_id="0", action_id="0"):
    ...
    try:
        ...
        request_body = request.get_json(force=True)
        workflow_for_item_application = request_body.get(
            "workflow_for_item_application", ""
        )
        terms_without_contents = request_body.get("terms_without_contents", "")
        is_display_item_application_button = request_body.get(
            "is_display_item_application_button", False
        )
        terms_description_without_contents = request_body.get(
            "terms_description_without_contents", ""
        )
...
@workflow_blueprint.route(
    "/get_feedback_maillist/<string:activity_id>", methods=["GET"]
)
@login_required
    ...
    try:
        ...
        if action_feedbackmail:
            ...
            if not isinstance(mail_list, list):
                res = ResponseMessageSchema().load(
                    {"code": -1, "msg": "mail_list is not list"}
                )
                return jsonify(res.data), 400
            temp_list = []
            added_user = []
            ...
            mail_list += temp_list
            res = GetFeedbackMailListSchema().load(
                {"code": 1, "msg": _("Success"), "data": mail_list}
            )
            return jsonify(res.data), 200
        else:
...
def is_user_locked():
    ...
    if cur_locked_val:
        ...
        if act is None or act.activity_status in [
...
def user_lock_activity(activity_id="0"):
    ...
    if cur_locked_val:
        ...
        if act is None or act.activity_status in [
            ...
            update_cache_data(cache_key, activity_id, timeout)
    ...
    res = {
        ...
        "err": err or "",
        "activity_id": cur_locked_val,
    }
    return jsonify(res), 200


@workflow_blueprint.route(
    "/activity/user_unlock/<string:activity_id>", methods=["POST"]
)
@login_required
...
def lock_activity(activity_id="0"):
    ...
    def is_approval_user(activity_id):
        workflow_activity_action = ActivityAction.query.filter_by(
            activity_id=activity_id, action_status=ActionStatusPolicy.ACTION_DOING
        ).one_or_none()
    ...
    if cur_locked_val and not is_approval_user(activity_id):
        if locked_value != cur_locked_val:
            ...
            if act is None or act.activity_status in [
                ...
                update_cache_data(cache_key, locked_value, timeout)
    else:
    ...
    locked_by_email, locked_by_username = get_account_info(locked_value.split("-")[0])
    if locked_by_email is None or locked_by_username is None:
        current_app.logger.error(
            "lock_activity: can not get locked_by_email or locked_by_username"
        )
        res = ResponseMessageSchema().load(
            {"code": -1, "msg": "can not get user locked"}
        )
        return jsonify(res.data), 500
    res = ResponseLockSchema().load(
        {
            "code": 200,
            "msg": "" if err else _("Success"),
            "err": err or "",
...
def check_approval(activity_id="0"):
    ...
    if not check_flg:
        ...
        res = ResponseMessageSchema().load({"code": -1, "msg": "arguments error"})
        return jsonify(res.data), 400
    response = {"check_handle": -1, "check_continue": -1, "error": 1}
    try:
...
@workflow_blueprint.route(
    "/send_mail/<string:activity_id>/<string:mail_id>", methods=["POST"]
)
@login_required
...
def usage_report():
    ...
    activities, _, _, _, _, _ = activity.get_activity_list(
        conditions=conditions, is_get_all=True
    )
    get_workflow_item_type_names(activities)
    activities_result = []
...
def download_activitylog():
    ...
    if "activity_id" in request.args:
        tmp_activity = activity.get_activity_by_id(
        ...
        conditions = filter_all_condition(request.args)
        activities, _, _, _, _, _ = activity.get_activity_list(
            conditions=conditions, activitylog=True
        )
...
def clear_activitylog():
    ...
    # delete a activity
    if "activity_id" in request.args:
        del_activity = activity.get_activity_by_id(
            activity_id=request.args.get("activity_id")
        )
        if del_activity == None:
            return jsonify(code=-1, msg="no activity error"), 400
        if del_activity.activity_status in [
            ActivityStatusPolicy.ACTIVITY_MAKING,
            ActivityStatusPolicy.ACTIVITY_BEGIN,
        ]:
        ...
        try:
            with db.session.begin_nested():
                workflow_activity_action.query.filter_by(
                    activity_id=del_activity.activity_id
                ).delete()
    ...
    # delete all filitering activity
        ...
        for del_activity in activities:
            if del_activity.activity_status in [
                ...
                if not result:
                    return (
                        jsonify(code=-1, msg=str(DeleteActivityFailedRESTError())),
                        400,
                    )
        ...
        try:
            with db.session.begin_nested():
                for activty in activities:
                    workflow_activity_action.query.filter_by(
                        activity_id=activty.activity_id
                    ).delete()
...
class ActivityActionResource(ContentNegotiatedMethodView):
    ...
    def activity_information(self, activity):
        ...
        """
        response = {"activityId": activity.activity_id, "email": None, "status": None}

        user = User.query.get(activity.activity_login_user)
        response["email"] = user.email if user else ""

        status = ActionStatusPolicy.describe(ActionStatusPolicy.ACTION_DOING)
        if activity.activity_status == ActivityStatusPolicy.ACTIVITY_FINALLY:
            status = ActionStatusPolicy.describe(ActionStatusPolicy.ACTION_DONE)
        elif activity.activity_status == ActivityStatusPolicy.ACTIVITY_CANCEL:
            status = ActionStatusPolicy.describe(ActionStatusPolicy.ACTION_CANCELED)
        response["status"] = _(status)

        current_app.logger.info(
            "{}_{}_{}_{}_FINISH_{}".format(
                self._prefix,
                request.method,
                current_user.email,
                self._time,
    ...
    def __init__(self, *args, **kwargs):
        """Constructor."""
        super(ActivityActionResource, self).__init__(*args, **kwargs)
        self._prefix = current_app.config.get("WEKO_WORKFLOW_GAKUNINRDM_PREFIX")
    ...
    def post(self):
        ...
        """
        current_app.logger.info(
            "{}_{}_{}_{}_REQUEST".format(
                self._prefix, request.method, current_user.email, self._time
            )
        )
        ...
        item = (
            ...
            if check_result.get("list_record")
            else None
        )
        if check_result.get("error") or not item or item.get("errors"):
            if check_result.get("error"):
                self.logging_error("check_import_items", check_result.get("error"))
        ...
        if not import_result["success"]:
            self.logging_error("import_items_to_system", import_result["success"])
            raise InvalidInputRESTError()
        ...
        try:
            ...
            pid = PersistentIdentifier.query.filter_by(
                pid_type="recid", pid_value=import_result.get("recid")
            ).first()
            activity = self.activity.init_activity(
                post_activity, item_id=pid.object_uuid
            )
            self.activity.update_title(activity.activity_id, item.get("item_title"))
        except Exception as ex:
...
depositactivity_blueprint.add_url_rule(
    "/<string:activity_id>",
    view_func=ActivityActionResource.as_view("workflow_activity_action"),
    methods=["GET", "DELETE"],
)
...
depositactivity_blueprint.add_url_rule(
    "",
    view_func=ActivityActionResource.as_view("workflow_activity_new"),
    methods=["POST"],
)
...
def edit_item_direct(pid_value):
    ...
    if not current_user.is_authenticated:
        return redirect(
            url_for_security(
                "login", next="/workflow/edit_item_direct_after_login/" + pid_value
            )
        )
    else:
...
@workflow_blueprint.route(
    "/edit_item_direct_after_login/<string:pid_value>", methods=["GET"]
)
@login_required
def edit_item_direct_after_login(pid_value):
    ...
    if sessionstorage.redis.exists("pid_{}_will_be_edit".format(pid_value)):
        return (
            render_template(
                "weko_theme/error.html", error="This Item is being edited."
            ),
            400,
        )
    else:
    ...
    try:
        ...
        resolver = Resolver(
            pid_type="recid", object_type="rec", getter=record_class.get_record
        )
        ...
        if not deposit:
            return (
                render_template(
                    "weko_theme/error.html", error="Record does not exist."
                ),
                404,
            )
    except PIDDoesNotExistError as ex:
        return (
            render_template("weko_theme/error.html", error="Record does not exist."),
            404,
        )
    ...
    if not ItemTypes.get_latest():
        return (
            render_template(
                "weko_theme/error.html", error="You do not even have an ItemType."
            ),
            400,
        )
    ...
    if not item_type:
        return (
            render_template(
                "weko_theme/error.html", error="Dependency ItemType not found."
            ),
            400,
        )
    ...
    if post_workflow:
        ...
        post_workflow = activity.get_workflow_activity_by_item_id(recid.object_uuid)
        ...
        if not workflow:
            return (
                render_template(
                    "weko_theme/error.html", error="Workflow setting does not exist."
                ),
                400,
            )
    ...
    try:
        ...
        db.session.rollback()
        return (
            render_template("weko_theme/error.html", error="An error has occurred."),
            500,
        )
    except BaseException as ex:
        import traceback
    ...
    return redirect(
        url_for("weko_workflow.display_activity", activity_id=rtn.activity_id)
    )
