...
from weko_records.models import ItemMetadata, ItemReference, ItemTypeName, ItemType
...
def test_check_require_itemtype_mapping(client, users,db, mocker):
    mocker.patch("weko_workflow.views.db.session.remove")
    ...
    url = url_for("weko_workflow.check_require_itemtype_mapping",itemtype_id=item_type.id)
    
    login(client=client, email=users[2]['email'])
    # mappingなし、必須なし
    res = client.get(url)
    assert res.status_code == 200
    assert json.loads(res.get_data(as_text=True)) == ["title"]
