...
class TestWekoIndexer:
    ...
    def test_update_relation_version_is_last(self, base_app, es_records):
        with base_app.test_request_context():
            with patch('weko_deposit.api.weko_logger') as mock_logger:
                ...
                expected_result = {'_index': 'test-weko-item-v1.0.0', '_id': '{}'.format(
                    pid.object_uuid), '_version': 2, 'result': 'noop', '_shards': {'total': 0, 'successful': 0, 'failed': 0}}
                assert all(item in indexer.update_relation_version_is_last(relations_ver).items() for item in expected_result.items()) 
...
class TestWekoDeposit():
    ...
    def test_publish_without_commit(self, app, base_app, db, location):
        with patch('weko_deposit.api.weko_logger') as mock_logger:
            with app.test_request_context():
