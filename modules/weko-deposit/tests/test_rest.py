...
def test_depid_item_put_acl_guest(client, deposit):
    ...
    res = client.put(url, data=json.dumps(input),
                     content_type='application/json')
    # 未認証では編集できない。以前は認可が無く 200 で通っていた。
    # CNMV なので 302 を返さず 401 を raise する。
...
# edit_mode='upgrade' の経路は convert_item_metadata を通り、そこで
# Indexes.get_path_list() がレコードの path に対応するインデックスを
# 引けないと PIDResolveRESTError になる。db_index でインデックスを作る。
def test_depid_item_put(client, users, db_index, es_records, mocker):
...
def test_depid_item_post_guest(client, deposit):
    ...
    res = client.post(url,
                      ...
                      content_type='application/json')
    # 未認証では depid の存在・作成日時・編集画面リンクを引けない。
    assert res.status_code == 401
...
def test_put(app, client, users, records):
    # 認可を入れたので、バージョン更新の機能テストもログインしてから叩く。
    # ここで見たいのは newversion / call_external_system の呼ばれ方であって
    # 認可の可否ではないため、確実に通る sysadmin を使う。
    login_user_via_session(client=client, email=users[2]['email'])
    ...
    with patch("weko_deposit.rest.WekoDeposit.newversion", return_value=version):
        ...
        with patch("weko_deposit.rest.WekoRecord.get_record_by_pid", mock_record):
            with patch("weko_records_ui.external.call_external_system") as mock_external:
