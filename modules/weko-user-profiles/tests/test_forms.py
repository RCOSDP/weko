...
# .tox/c1/bin/pytest --cov=weko_user_profiles tests/test_forms.py::TestProfileForm::test_validate_username -vv -s --cov-branch --cov-report=term --basetemp=/code/modules/weko-user-profiles/.tox/c1/tmp
    ...
    def test_init_storage_fields_removed_when_disabled(self, app):
        ...
        with app.app_context():
            ...
            # Verify storage-related fields are removed. WTForms' del drops
            # the field from the form but leaves the attribute set to None, so
            # hasattr() stays True; membership is what actually changes.
            assert 'access_key' not in form
            assert 'secret_key' not in form
            assert 's3_endpoint_url' not in form
            assert 's3_region_name' not in form
