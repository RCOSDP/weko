...
@pytest.mark.xfail(
    raises=TypeError,
    reason=(
        "invenio_resourcesyncclient bug, not a test one: utils.process_item "
        "calls mapper.map() with no argument, but JPCOARMapper.map takes a "
        "required `version` (invenio_oaiharvester/harvester.py:1502). "
        "invenio-oaiharvester's own caller passes it "
        "(tasks.py:226), and so does weko-search-ui; only this module was "
        "left behind, so resyncing a JPCOAR record raises TypeError. Fixing "
        "it means changing invenio_resourcesyncclient.utils."
    ),
)
