...
def test_check_duplicate_mapping(db_itemtype6):
    ...
    # check_duplicate_mapping keeps only the keys that are in the item type's
    # table_row - plus the system ones it appends to that list itself - and
    # drops the rest from the dict it was handed. 'item_test' is one of those.
    # Read table_row before the call: the function extends the list in place.
    expected_keys = set(item_type.render['table_row']) | {
        'pubdate', 'system_file', 'system_identifier_doi',
        'system_identifier_hdl', 'system_identifier_uri',
    }
    ...
    assert set(data_mapping_copy) == set(data_mapping) & expected_keys
    assert 'item_test' not in data_mapping_copy
