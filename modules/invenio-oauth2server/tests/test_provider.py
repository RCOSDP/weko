...
def test_client_salt(provider_fixture):
    ...
    with app.app_context():
        with db.session.begin_nested():
            ...
            client.gen_salt()
            assert (
                len(client.client_id) == app.config["OAUTH2SERVER_CLIENT_ID_SALT_LEN"]
            )
            assert (
                len(client.client_secret)
...
def test_refresh_flow(provider_fixture):
    ...
    with app.test_request_context():
        ...
        with app.test_client() as client:
            ...
            r = client.get(
                url_for(
                    "invenio_oauth2server.info", access_token=json_resp["access_token"]
                )
            )
            assert r.status_code == 200
    ...
    with app.test_request_context():
        ...
        with app.test_client() as client:
            ...
            for client_id in ["dev", "confidential"]:
                ...
                assert json.loads(r.get_data()).get("scopes") == ["test:scope"]
...
def test_client_flow(provider_fixture):
    ...
    with app.test_request_context():
        with app.test_client() as client:
            ...
            user = User.query.filter_by(email="inactive@inveniosoftware.org").first()
            user.active = True
            db.session.commit()
            ...
            assert r.status_code == 200
            assert (
                json.loads(r.get_data()).get("client") == "confidential-user-inactive"
            )
...
def test_personal_access_token(provider_fixture):
    ...
    with app.test_request_context():
        with app.test_client() as client:
            ...
            r = client.get(
                url_for("invenio_oauth2server.ping"),
                query_string="access_token={0}".format(app.personal_token3),
            )
            assert r.status_code == 401
...
def test_resource_auth_methods(provider_fixture):
    ...
    with app.test_request_context():
        with app.test_client() as client:
            ...
            r = client.get(
                url_for("invenio_oauth2server.ping"),
                headers=[("Authorization", "Bearer {0}".format(app.personal_token))],
            )
            assert r.status_code == 200
            assert json.loads(r.get_data()) == dict(ping="pong")
...
def test_oauthlib_urldecoding_issue(api_app_with_test_view, query_string, valid_qs):
...
def test_expired_refresh_flow(provider_fixture):
    ...
    with app.test_request_context():
        with app.test_client() as client:
            ...
            r = client.get(
                url_for(
                    "invenio_oauth2server.info", access_token=resp_json["access_token"]
                )
            )
            assert r.status_code == 200
