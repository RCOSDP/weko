...
def test_ext_class_init(app):
    ...
    assert 1 == 1
# ===========================def reset_filters():=====================================
@pytest.mark.parametrize('users_index, status_code, mock_setup, expected_response', [
    (0, 200, 
     {'return_value': None},  
    ...
    (0, 500, 
     ...
     {"status": "error", "message": "Failed to reset default conditions due to database error: Database error"}),

    (0, 500, 
     {'side_effect': Exception("Unexpected error")},  
     {"status": "error", "message": "Unexpected error occurred: Unexpected error"}),
])
def test_reset_filters(client, users, users_index, status_code, mock_setup, expected_response, workspaceData):
    login(client=client, email=users[users_index]['email'])

    if status_code == 200 and expected_response.get("message") == "Successfully reset default conditions.":
        if workspaceData:
            mock_setup['return_value'] = workspaceData[0]  
        else:
            raise ValueError("workspaceData is empty. Please ensure data is inserted correctly.")
...
# ===========================def save_filters():=====================================
@pytest.mark.parametrize('users_index, status_code, mock_setup, post_data, expected_response', [
    (1, 200, 
     {'return_value': None},  
     {"filters": {"key": "value"}},  
    ...
    (1, 500, 
     {'side_effect': SQLAlchemyError("Database error")},  
     {"filters": {"key": "value"}},  
     {"status": "error", "message": "Failed to save default conditions due to database error: Database error"}),
...
def test_save_filters(client, users, users_index, status_code, mock_setup, post_data, expected_response, workspaceData, db):
    login(client=client, email=users[users_index]['email'])

    if status_code == 200 and expected_response.get("message") == "Successfully saved default conditions.":
            ...
            with patch('weko_workspace.views.db.session.add') as mock_add:
                with patch('weko_workspace.views.db.session.commit') as mock_commit:
                    ...
                    if mock_setup.get('return_value') is not None:
                        ...
                        workspace_calls = [call for call in mock_add.call_args_list if isinstance(call[0][0], WorkspaceDefaultConditions)]
...
def test_update_workspace_status_management(client, users, users_index, status_code, mock_setup, post_data, expected_response, workspaceData):
    login(client=client, email=users[users_index]['email'])
    ...
    with patch('weko_workspace.views.get_workspace_status_management') as mock_get_status:
            ...
            with patch('weko_workspace.views.update_workspace_status') as mock_update:
                ...
                if status_code == 200:
                    if mock_setup['return_value'] is None:
                        ...
                        if post_data['type'] == "1":
                            assert call_args['is_favorited'] == post_data.get('favoriteSts', False)
                            assert call_args['is_read'] == False
                        elif post_data['type'] == "2":
                            assert call_args['is_favorited'] == False
                            assert call_args['is_read'] == post_data.get('readSts', False)
                    else:
                        ...
                        assert call_args['recid'] == post_data['itemRecid']
                        if post_data['type'] == "1":
                            assert call_args['is_favorited'] == post_data.get('favoriteSts')
...
# # TODO 
# # method = 'GET', url = '/api/worksapce/search', response = None
# # error = NewConnectionError('<urllib3.connection.HTTPConnection object at 0x7f1deefcb908>: Failed to establish a new connection: [Errno -2] Name or service not known',)
# @pytest.mark.parametrize('users_index, method, mock_setup, post_data, expected_items_count', [
#     # 场景 1: GET 请求，成功获取项目列表（无筛选条件）
#     (0, 'GET', 
#      {
#          'es_data': {
#              "hits": {
...
#                              "_item_metadata": {"file": [], "peer_review": False}
#                          }
#                      }
#                  ]
#              }
...
#      0),  # 期望返回 0 个项目
# ])
# def test_get_workspace_itemlist(client, users, users_index, method, mock_setup, post_data, expected_items_count):
#     login(client=client, email=users[users_index]['email'])
...
#     with patch('weko_workspace.views.get_workspace_filterCon') as mock_filter:
#         mock_filter.return_value = (mock_setup['filter_con'], True) if mock_setup['filter_con'] is not None else (None, False)

#     with patch('weko_workspace.views.get_workspace_status_management') as mock_status:
#         mock_status.return_value = mock_setup['status_data']

#     with patch('weko_workspace.views.get_accessCnt_downloadCnt') as mock_access:
#         mock_access.return_value = mock_setup['access_data']

#     with patch('weko_workspace.views.get_item_status') as mock_item_status:
#         mock_item_status.return_value = mock_setup['item_status']

#     with patch('weko_workspace.views.extract_metadata_info') as mock_extract:
#         mock_extract.return_value = ([], True)  # filelist, peer_reviewed

#     with patch('weko_workspace.views.get_userNm_affiliation') as mock_user_info:
#         mock_user_info.return_value = ("Test User", "Test Affiliation")

#     with patch('weko_workspace.views.FeedbackMailList.get_feedback_mail_list') as mock_feedback:
#         mock_feedback.return_value = {users[users_index]['email']: True}

#     with patch('weko_workspace.views.merge_default_filters') as mock_merge:
#         mock_merge.return_value = post_data if post_data else {}

#     with patch('weko_workspace.views.render_template') as mock_render:
...
#             assert item['title'] == mock_setup['es_data']['hits']['hits'][0]['metadata']['title'][0]
...
#             assert item['fbEmailSts'] == True  # 用户在 FeedbackMailList 中

#         # 验证 defaultconditions
#         if post_data:
